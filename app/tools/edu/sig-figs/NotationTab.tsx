...
/* NotationTab — '과학적 표기' 탭 (구 /tools/edu/sci-units 과학 단위 변환기를 흡수, ?tab=notation)
   SigFigsClient가 next/dynamic으로 지연 로드한다 — 기본 탭(유효숫자·반올림) 번들에 포함되지 않는다.
   하위 보기 3종: 표기·접두어 변환 / 과학 스케일 단위 / 물리 상수표. 계산·데이터는 notationData.ts(원본 그대로). */
import { useMemo, useState } from 'react'
import s from './notation.module.css'
import {
...
type View = 'notation' | 'units' | 'constants'

export default function NotationTab() {
  const [view, setView] = useState<View>('notation')

  return (
    <>
      <div className={s.segRow} role="tablist" aria-label="과학적 표기 도구">
        <button type="button" role="tab" aria-selected={view === 'notation'} className={`${s.seg} ${view === 'notation' ? s.segActive : ''}`} onClick={() => setView('notation')}>접두어·표기 변환</button>
        <button type="button" role="tab" aria-selected={view === 'units'} className={`${s.seg} ${view === 'units' ? s.segActive : ''}`} onClick={() => setView('units')}>과학 스케일 단위</button>
        <button type="button" role="tab" aria-selected={view === 'constants'} className={`${s.seg} ${view === 'constants' ? s.segActive : ''}`} onClick={() => setView('constants')}>물리 상수표</button>
      </div>

      {view === 'notation' && <NotationView />}
      ...
      <div className={s.card}>
        ...
        <div className={s.groupTabs}>
          {SCI_UNIT_GROUPS.map(g => (
            <button key={g.id} type="button" aria-pressed={groupId === g.id} className={`${s.groupTab} ${groupId === g.id ? s.groupTabActive : ''}`} onClick={() => changeGroup(g.id)}>
              {g.name}
            </button>
          ))}
        </div>
