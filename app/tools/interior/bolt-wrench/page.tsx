...
const FAQ_LD = [
  ...
  { "q":"차량 휠너트는 몇 Nm로 조여야 하나요?","a":"차종마다 매우 다릅니다. 일반적인 가이드: 국산 경차·소형 80~100Nm, 중형 100~120Nm, SUV·대형 120~140Nm, 트럭·중장비 200Nm 이상. 반드시 차량 매뉴얼·휠 제조사 사양 확인이 우선이며, 토크렌치로 별 모양(star pattern) 순서로 2~3회 나눠 조이세요. 휠너트는 안전 직결 부품이므로 자신 없으면 정비공장을 이용하세요." }
]

export default function BoltWrenchPage() {
  return (
    <ToolPage width={880} slug="/tools/interior/bolt-wrench">
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          ④ 그래도 안 되면 토치로 가열(주변 가연물 제거·소화 수단 준비, 도장·플라스틱 변형 주의 — <strong>침투제 분사 직후 가열 금지</strong>) →
      ...
      <details style={faqDetails}>
        <summary style={faqSummary}>Q10. 차량 휠너트는 몇 Nm로 조여야 하나요?</summary>
        <p style={faqAnswer}>
