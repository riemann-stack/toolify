...
const FAQ_LD = [
  ...
  { "q":"토크렌치 꼭 써야 하나요?","a":"고급 부품·안전 부품은 필수입니다. 자전거 카본 시트포스트·스템(5~8Nm), 자동차 휠너트(80~140Nm 차종별), 엔진 헤드볼트 등은 적정 토크 미만이면 풀리고, 초과면 부품 파손·균열이 발생합니다. 일반 가구 조립처럼 손감각으로 충분한 곳도 있지만, 한 번 정확한 토크를 익히면 손감각도 정확해집니다. 토크렌치는 조일 때만 사용하고 풀 때는 일반 핸들을 쓰세요." },
  { "q":"녹슨 볼트가 풀리지 않을 때는?","a":"순서대로: ① 침투제(WD-40·PB Blaster)를 충분히 뿌리고 10~30분 대기 → ② 머리를 망치로 가볍게 두드려 충격 전달 → ③ 역방향으로 살짝 조였다가 풀기(녹 균열) → ④ 그래도 안 되면 토치로 가열(주변 도장·플라스틱 주의) → ⑤ 마지막 수단은 볼트 익스트랙터·드릴링. 한 사이즈 큰 스패너는 둥글림이 더 심해지므로 금물입니다." },
  { "q":"와셔는 왜 같이 쓰나요?","a":"① 하중 분산으로 모재 손상 방지(목재·연질 금속에 특히 중요), ② 풀림 방지(스프링와셔), ③ 접지·전기 도통(이붙이와셔), ④ 틈새 메움·간격 조정. 와셔는 볼트 머리 쪽 + 너트 쪽 둘 다 넣는 것이 표준이며, 스프링와셔는 평와셔 안쪽(나사산 쪽)에 넣습니다." },
  { "q":"나일론락너트 재사용이 가능한가요?","a":"비권장입니다. 한 번 체결하면 나일론 인서트가 영구 변형되어 풀림 방지력이 30~50% 감소합니다. 안전 부품(휠허브·서스펜션·핸들바 등)은 반드시 새것을 사용하세요. 임시 분해·조립이라면 2~3회 재사용은 큰 무리는 아니지만, 풀림 토크가 분명히 떨어진 너트는 폐기가 정답입니다. 반복 분해가 잦은 곳은 더블너트·스프링와셔 조합으로 대체하세요." },
  { "q":"스테인리스 볼트는 토크가 다른가요?","a":"네, 보통 일반 강 8.8 등급의 60~70% 정도가 안전 토크입니다. 스테인리스(SUS304·SUS316)는 강도등급 표기가 A2-70·A4-80으로 다르며, 마찰계수도 높아 긁어붙임(galling) 현상이 잘 일어납니다. 체결 시 항상 항착방지제(anti-seize)나 윤활제를 바르고, 천천히 균등하게 조이세요. 정확한 값은 제조사 데이터시트를 우선 확인하세요." },
  { "q":"경량 차량 휠너트는 몇 Nm로 조여야 하나요?","a":"차종마다 매우 다릅니다. 일반적인 가이드: 국산 경차·소형 80~100Nm, 중형 100~120Nm, SUV·대형 120~140Nm, 트럭·중장비 200Nm 이상. 반드시 차량 매뉴얼·휠 제조사 사양 확인이 우선이며, 토크렌치로 별 모양(star pattern) 순서로 2~3회 나눠 조이세요. 휠너트는 안전 직결 부품이므로 자신 없으면 정비공장을 이용하세요." }
]
