...
const FAQ_LD = [
              {
                q: '커튼 주름은 몇 배가 좋은가요?',
                a: '가장 흔히 쓰는 기준은 <strong>2배 주름</strong>입니다(정해진 규격이 아니라 업계 관행). 봉 길이가 200cm라면 커튼 전체 폭은 400cm가 됩니다. 1.5배는 가벼운 자연 주름, 2.5~3배는 호텔 스타일의 매우 풍성한 주름입니다. 일반 가정 거실·침실은 2배가 가장 안정적이고, 시어 커튼은 1.5~2배, 암막 커튼은 2~2.5배를 권장합니다.',
              },
              {
              ...
              },
              {
                ...
                a: '가장 무난하게 많이 고르는 길이는 <strong>"바닥에서 5cm 위"</strong>입니다. 깔끔한 인상을 주고 청소가 편리합니다. 호텔이나 고급 인테리어를 원한다면 "바닥에 닿거나 +15cm 풀링"을 선택할 수 있지만 청소가 어렵고 먼지가 쌓이기 쉽습니다. 창문이 작거나 라디에이터·가구가 있는 경우 "창문 + 10cm" 짧은 길이도 좋습니다.',
              },
              {
              ...
              {
                q: '원단은 몇 폭이 필요한가요?',
                a: '폭 140cm 원단을 기준으로 <strong>커튼 전체 폭(봉 길이 × 주름 배수) ÷ 140 → 올림</strong>으로 셉니다. 주름 배수는 2~2.5배가 통용 관행입니다. 예를 들어 창 220cm에 본 계산기 기준 봉 길이 250cm, 2배 주름이면 전체 폭 500cm → 500 ÷ 140 ≈ 3.57 → <strong>4폭</strong>, 2.5배면 5폭입니다. 길이 방향의 상단 헤딩·밑단 시접 여유는 봉제 방식과 업체마다 다르며, 본 계산기는 헴 10cm를 더한 원단 주문 길이를 함께 표시합니다.',
              },
            ]
...
export default function CurtainBlindPage() {
  return (
    <ToolPage width={760} slug="/tools/interior/curtain-blind">
      ...
      <UpdatedMeta
        date="2026년 7월"
        basis="사이즈 공식은 업계 통용 관행(커튼봉 = 창 폭 + 30cm · 커튼 폭 = 봉 길이 × 주름 배수 · 블라인드 인사이드 폭 −1cm·높이 −0.5cm, 아웃사이드 +10cm) · 차광 1~3급 구간은 일본 인테리어패브릭협회(NIF, JIS L 1055 A법) 기준 · 한국은 KS K 0819 차광성 시험방법"
        sources={[
          { label: '산겟츠 — 커튼 기능 가이드(NIF 차광 등급)', href: 'https://www.sangetsu.co.jp/style/curtain_choose04.html' },
          { label: 'e나라표준인증 — KS K 0819 커튼의 차광성 시험방법', href: 'https://www.standard.go.kr/KSCI/standardIntro/getStandardSearchView.do?menuId=503&ksNo=KSK0819&tmprKsNo=KSK0819' },
        ]}
      />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '8px' }}>
            {[
              ...
              { p: '×2.0', c: 'var(--accent)', t: '가장 흔한 기준',  d: '풍성, 일반 거실·침실' },
        ...
        <div>
          ...
          <p className="g-p">
            원단을 끊어 맞춤 제작할 때는 <strong>'몇 폭(幅)이 필요한가'</strong>로 주문합니다. 국내 유통 커튼 원단은 폭 140cm 안팎의 대폭 제품이 흔한데(업계 관행 — 실제 폭은 원단마다 다르니 표기 확인), 폭수는 이렇게 셉니다.
          </p>
          <div style={{
            background: 'var(--bg2)',
            border: '1px solid var(--border)',
            ...
            <div><span style={{ color: 'var(--muted)' }}>필요 폭수</span> = 커튼 전체 폭(봉 길이 × 주름 배수) ÷ 원단 폭 140cm → <span style={{ color: 'var(--muted)' }}>소수점 올림</span></div>
            <div><span style={{ color: 'var(--muted)' }}>주름 배수</span> = 2~2.5배 <span style={{ color: 'var(--muted)' }}>(통용 관행 — 본 계산기 기본값 2배)</span></div>
          </div>
          <p className="g-p">
            예를 들어 거실 창 220cm — 본 계산기는 창 폭에 좌우 여유 30cm를 더한 <strong>봉 길이 250cm</strong>에 배수를 곱합니다. 2배 주름이면 전체 폭 500cm → 500 ÷ 140 ≈ 3.57 → <strong>올림 4폭</strong>(양쪽 한 쌍이면 패널당 2폭씩), 2.5배면 625 ÷ 140 ≈ 4.46 → <strong>5폭</strong>. 창 폭 220cm에 바로 배수를 곱하는 간이 방식이면 440 ÷ 140 ≈ 3.14 → 역시 4폭이지만, 기준에 따라 경계에서 1폭이 갈릴 수 있으니 애매하면 큰 쪽이 안전합니다.
          </p>
          <p className="g-p">
            길이 방향은 완성 길이에 상단 헤딩(봉집·심지)과 밑단 접기 시접을 더해 재단하는데, 여유량은 봉제 방식·업체마다 다릅니다(통용 관행). 본 계산기는 완성 길이에 <strong>헴 10cm</strong>를 더한 '원단 주문 길이'를 결과에 함께 표시합니다. 무늬 원단은 폭끼리 무늬를 맞추는 여유(리피트)가 추가로 필요합니다.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="g-p" style={{ marginTop: 12 }}>
            주문 전 상품 폭이 <strong>원단 폭 기준인지 완성 폭 기준인지</strong> 확인하세요. 본 계산기의 '커튼 전체 폭'은 주름 배수를 곱한 원단 기준 폭입니다.
          </p>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {[
              { t: '창문형 (창문 + 10cm)',         d: '창문만 가리는 짧은 커튼. 작은 창문·주방·욕실에 적합.', c: 'var(--cyan-600)' },
              { t: '무릎형 (바닥 ~ 무릎)',          d: '거실 창에는 잘 쓰지 않고, 창 아래에 가구·수납장이 있는 곳에 사용.', c: 'var(--orange-600)' },
              { t: '바닥형 (바닥 5cm 위) — 가장 무난', d: '깔끔한 인상, 청소 편함. 거실·침실 모두 적합.', c: 'var(--accent)' },
              { t: '바닥 닿기 / 풀링 (+15cm)',     d: '호텔·고급 인테리어. 우아하지만 청소 어려움.', c: 'var(--amethyst)' },
            ].map((s, i) => (
              <div key={i} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderLeft: `3px solid ${s.c}`, borderRadius: 'var(--radius-m)', padding: '12px 16px' }}>
                <div>
        ...
        <div>
          ...
          <ul className="g-list">
            <li>창문 <strong>3지점(좌·중·우) 측정</strong> 후 가장 작은 값 사용 — 인사이드 마운트는 가장 좁은 지점보다 크게 주문하면 틀 안에 들어가지 않습니다</li>
            <li>줄자는 <strong>수평·수직 정확히</strong> (기울어지면 1~2cm 오차)</li>
            <li>창문 가까이의 <strong>가구·에어컨·콘센트·창문 손잡이 위치 확인</strong> — 블라인드 하단바나 커튼 자락이 걸리지 않는지</li>
            <li>오래된 집에서 창문 모서리가 <strong>직각이 아닌 경우</strong> 추가 여유 필요</li>
            <li>인사이드 마운트는 <strong>창문틀 깊이 6cm 이상</strong> 확인 필수</li>
            <li>치수는 <strong>cm 단위, 가로 × 세로 순서</strong>로 적어 두기 — 주문서에서 가로·세로를 바꿔 적는 실수가 잦습니다</li>
          </ul>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '10px' }}>
            {[
              { t: '커튼',       c: 'var(--accent)', d: '거실·침실, 포근한 느낌, 풍성한 인테리어, 두꺼운 원단은 창가 냉기·소음도 어느 정도 막아 줌' },
              { t: '블라인드',  c: 'var(--cyan-600)',       d: '사무실·미니멀, 큰 창문(버티칼), 빛 양 세밀 조절' },
              { t: '롤스크린',   c: 'var(--yellow-700)',       d: '욕실·주방·작은 창, 단순한 인테리어, 가성비' },
              { t: '로만쉐이드', c: 'var(--amethyst)',       d: '커튼 분위기 + 블라인드 기능, 침실·소형 창문' },
            ].map((s, i) => (
        ...
        <div>
          ...
          <p className="g-p">
            쇼핑몰의 '차광 1급·2급' 표기 — 널리 쓰이는 1~3급 구분은 <strong>일본 인테리어패브릭협회(NIF)</strong>가 JIS L 1055 A법(차광성 시험)에 따라 정한 업계 기준이 국내에 통용된 것입니다(2026-07 확인 기준).
          </p>
          <div className="tableScroll">
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', minWidth: 460 }}>
              <thead>
          ...
          <p className="g-p" style={{ marginTop: 12 }}>
            2018년 8월부터 NIF는 1급을 다시 A++·A+·A·B·C 5단계로 세분화했습니다. 한국에는 <strong>KS K 0819 「커튼의 차광성 시험방법」</strong>이 있으며(1989 제정 · 2018 개정 · 2023-07-31 최종 확인 고시, 현행 유효) 확인되는 것은 이 '시험방법' 표준의 실재까지입니다 — 1~3급 구간 수치의 출처로 확인되는 것은 일본 NIF 기준이므로, 등급 숫자를 'KS 등급'으로 단정한 상품 설명은 걸러 읽는 것이 좋습니다.
          </p>
          <p className="g-note">
            출처:{' '}
            <a href="https://www.sangetsu.co.jp/style/curtain_choose04.html" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--muted)', textDecoration: 'underline', textUnderlineOffset: 2 }}>산겟츠 커튼 기능 가이드(NIF 구간)</a>
            {' · '}
            <a href="https://www.standard.go.kr/KSCI/standardIntro/getStandardSearchView.do?menuId=503&ksNo=KSK0819&tmprKsNo=KSK0819" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--muted)', textDecoration: 'underline', textUnderlineOffset: 2 }}>e나라 표준인증 — KS K 0819</a>
          </p>
        </div>
