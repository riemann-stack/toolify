...
const FAQ_LD = [
  { "q":"1.5sq 전선은 몇 A까지 가능한가요?","a":"HIV 동선·30°C·기중 직접고정(참조 부설방법 C) 기준 약 19A입니다(단상 2선 부하 기준, 삼상 3선 부하는 약 17.5A). 허용전류는 부설방법(관내·매설·기중)에 따라 달라지며, 이 값은 KEC가 채택한 IEC 60364-5-52 표의 방법 C를 기준선으로 씁니다. 여기에 보정계수(주위온도·다회로 묶음·매설)를 곱하면 70~85% 수준으로 떨어집니다. 예: 40°C에서 1.5sq → 19 × 0.87 = 약 16.5A. 콘센트 회로(20A 차단기)에는 일반적으로 2.5sq 이상을 권장합니다." },
  ...
  { "q":"EV 완속 충전기 7kW는 전선 굵기와 차단기?","a":"단상 220V 7kW = 약 32A (역률 1.0). 연속부하 125% 설계 여유로 40A 차단기 + 6sq HIV/F-CV가 한국 표준 조합입니다. 매설 구간은 F-CV(가교폴리에틸렌, 90°C)를 사용하고, EV 콘센트는 단독 분기 + RCBO(누전·과전류 통합) 30mA가 의무. 한전 신청·전기안전공사 검사가 별도로 필요합니다. 22kW 삼상(완속·중속)은 380V로 상(相)마다 전류가 나뉘어(상당 약 33A) 출력이 3배인데도 전선은 한 단계만 굵으면 됩니다. 이 계산기의 125% 여유 기준으로는 10sq + 50A이고, 6sq + 40A 조합도 과전류 보호 조건(부하전류 ≤ 차단기 ≤ 전선 허용전류)은 맞추지만 여유가 거의 없습니다. 진짜 급속(DC 50kW+)은 별도의 충전 방식입니다." },
  ...
  { "q":"누전차단기와 배선용차단기 차이는?","a":"배선용차단기(MCCB)는 과부하·단락(쇼트)으로부터 전선·기기를 보호 — 화재 예방. 누전차단기(ELCB)는 누전·감전으로부터 사람을 보호 — 감전사 예방. 가정용 분기 회로는 누전차단기 30mA·0.03초 의무이며, 욕실 등 물기 있는 장소는 고감도(15mA)를 추가하고, EV 충전 회로는 직류 누설까지 검출하는 RCD Type A/B 30mA(RCBO)를 사용합니다. 최근에는 둘을 합친 RCBO가 신축에서 표준화되고 있어요." },
  ...
  return (
    <ToolPage width={880} slug="/tools/interior/wire">
      ...
      <div style={card}>
        ...
        <ol style={{ paddingLeft: 18, margin: '12px 0 0', fontSize: 13, color: 'var(--muted)', lineHeight: 2 }}>
          ...
          <li>
            ...
            (HIV/IV·30°C·일반 부설 기준, 삼상은 도체 3개 부하 표라 같은 굵기도 허용전류가 조금 낮음)
          </li>
            <li>
            <strong style={{ color: 'var(--text)' }}>③ 차단기 = I × 1.25 이상</strong>{' '}
            의 가장 작은 표준값 (연속부하 125% 설계 여유) — 단, 차단기 정격 ≤ 전선 허용전류 (KEC 212 과전류 보호, IB≤In≤Iz). 차단기가 전선 허용전류보다 크면 전선을 한 단계 굵게 올립니다
          </li>
          <li>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          <strong>HIV 동선·30°C·기중 직접고정(참조 부설방법 C) 기준 약 19A</strong>입니다(단상 2선 부하 기준, 삼상 3선 부하는 약 17.5A).
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          한전 신청·전기안전공사 검사가 별도로 필요합니다. 22kW 삼상(완속·중속)은 380V로 상(相)마다 전류가 나뉘어(상당 약 33A)
          출력이 3배인데도 전선은 한 단계만 굵으면 됩니다. 이 계산기의 125% 여유 기준으로는 <strong>10sq + 50A</strong>이고,
          6sq + 40A 조합도 과전류 보호 조건(부하전류 ≤ 차단기 ≤ 전선 허용전류)은 맞추지만 여유가 거의 없습니다. 진짜 급속(DC 50kW+)은 별도의 충전 방식입니다.
        </p>
      </details>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          가정용 분기 회로는 누전차단기 30mA·0.03초 의무이며, 욕실 등 물기 있는 장소는 고감도(15mA)를 추가하고, EV 충전 회로는 직류 누설까지 검출하는 <strong>RCD Type A/B 30mA(RCBO)</strong>를 사용합니다.
