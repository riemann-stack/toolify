...
/* 인생 통계 탭 › 「기대수명·남은 시간」 보기 (구 /tools/date/life-time 통합, ?tab=life)
   ─ AgeClient가 next/dynamic으로 지연 로드 — 기본 탭(만 나이) 번들에 포함되지 않는다.
   ─ 생년월일은 부모(AgeClient) 공통 입력을 prop으로 받는다. 계산 로직(calc·action·remainingEx)은 원본 그대로.
   ─ 부모가 1초마다 리렌더(D-day 시계)하므로 memo로 감싸 주 단위 격자(최대 수천 rect) 재렌더를 막는다. */
 ...
 * 메인
 * ───────────────────────────────────────────────────────── */
function LifeTimePanel({ birth }: { birth: Date }) {
  /* 모드 상태 */
  ...
  /* 기준일(오늘 0시) — 생년월일은 부모 공통 입력(birth) 사용 */
  ...
  /* ─── 파생값 ─── */
  /* 부모가 new Date(y, m-1, d)로 만든 로컬 자정 Date(원본과 동일 생성식)이며 [년,월,일] memo라 참조가 안정적 */
  ...
  function handleShare() {
    const text = `${shareText()}\nyoutil.kr/tools/date/age?tab=life`
  ...
  return (
    <div className={styles.wrap}>
      {/* 진입 안내 (페이지 공통 면책은 AgeClient 상단 Disclaimer가 담당) */}
      <div className={styles.intro}>
        <span className={styles.introIcon}>ℹ️</span>
        <span>
          이 보기는 기대수명을 기준으로 시간을 환산해 보여주는 <strong style={{ color: 'var(--text)' }}>참고용 추정</strong>입니다.
          실제 수명이나 건강을 예측하지 않으며, 오늘을 더 의식적으로 살기 위한 가이드입니다.
          시간이 부담스럽게 느껴진다면 언제든 위의 「시간 통계」 보기로 돌아가거나 페이지를 닫으셔도 됩니다.
        </span>
      </div>
      ...
      {stage === 'mementoConfirm' && (
        <div className={styles.mementoConfirm}>
          ...
          <p className={styles.mementoConfirmBody}>
            이 모드는 <strong style={{ color: 'var(--amethyst)' }}>시간의 유한성을 직시</strong>하기 위한 모드입니다.
      ...
      {mounted && stage === 'show' && (
        <div className={styles.card}>
          <div className={styles.cardLabel}>
            <span>기대수명 설정</span>
            <span className={styles.cardLabelHint}>생년월일은 위 입력값 사용</span>
          </div>
              ...
              >
                {Array.from({ length: weekGrid.rows }).map((_, row) => (
                  Array.from({ length: weekGrid.cols }).map((__, col) => {
                    ...
                    // 지난 주 = 본문색 40% · 이번 주 = accent · 남은 주 = 본문색 6% (토큰 + fillOpacity)
                    const fill = isNow ? 'var(--accent)' : 'var(--text)'
                    const fillOpacity = isNow ? 1 : isPast ? 0.4 : 0.06
                    return (
                      <rect
                        key={`${row}-${col}`}
                        x={x}
...
export default memo(LifeTimePanel)
