...
const FAQ_LD = [
  ...
  {
    q: '실업급여는 한 달에 얼마 받나요?',
    a: `구직급여는 1일액 기준이라 한 달(30일)이면 1일액 × 30입니다. 하한액(${won(UI_DAILY_FLOOR_2026)}원)이면 약 <strong>${won(UI_DAILY_FLOOR_2026 * 30)}원</strong>, 상한액(${won(UI_DAILY_CAP_2026)}원)이면 약 <strong>${won(UI_DAILY_CAP_2026 * 30)}원</strong>입니다. 다만 매달 정확히 30일치가 지급되는 게 아니라 실업인정일 사이의 일수만큼 지급되므로, 위 금액은 참고용 월 환산치입니다. 30일 환산은 현행(2026년) 주 7일 지급 기준이며, 정부가 2027년부터 주 6일(무급휴일 제외) 지급으로 바꾸는 개편을 추진 중이라 확정되면 총액은 같아도 월 수령액은 줄고 받는 기간이 늘어납니다.`,
  },
  {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
        ...
        <div>
          ...
          <Callout tone="note" title={`이직일(마지막 근무일)이 ${NEXT_YEAR}년 1월 1일 이후라면`}>
            ...
            {!CAP_2027_CONFIRMED && <> {NEXT_YEAR}년 상한은 아직 고시되지 않아 이 계산기는 {UI_CAP_CONFIRMED_THROUGH}년 상한을 그대로 씁니다. 정부가 2026년 9월 1일 고용보험위원회에서 상한을 하한에 연동하고 지급 기준을 주 7일에서 주 6일(무급휴일 제외)로 바꾸는 개편안(총지급액·총 수급일수는 유지하되 월 수령액은 줄고 수급 기간은 길어짐, 실업급여 보험료율 근로자·사업주 각 0.9%→1.0%)을 확정했지만, 관련 법령 개정 전이라 이 계산기에는 반영하지 않았습니다. 최종 금액은 고용노동부 발표로 확인하세요.</>}
