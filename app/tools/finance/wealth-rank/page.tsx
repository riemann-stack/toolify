...
const W_TOP10_USD = Math.round(valueFromPercentile(GLOBAL, 90, true) / 1000) * 1000
const W_TOP1_USD = valueFromPercentile(GLOBAL, 99, true)
const usdK = (usd: number) => usd >= 1_000_000 ? `$${(usd / 1_000_000).toFixed(2).replace(/0$/, '')}M` : `$${usd.toLocaleString('en-US')}`
...
/** 계산 방식 예시 — 계산기와 같은 computeRank로 빌드 시점 계산 (단위: 만원) */
const EX_VALUES = [10_000, 30_000, 50_000, 100_000, 200_000]
const SEOUL = REGIONS.find((g) => g.id === 'seoul')!
const U39 = AGE_GROUPS.find((g) => g.id === 'u39')!
const RANK_ROWS = EX_VALUES.map((v) => ({
  v,
  nation: computeRank('nation', v),
  seoul: computeRank('region', v, SEOUL.id),
    ...
    <ToolPage width={760} slug="/tools/finance/wealth-rank">
      ...
      <UpdatedMeta
        ...
        sources={[
          ...
          { label: '국가통계포털 KOSIS(가계금융복지조사 통계표)', href: 'https://kosis.kr' },
        ]}
      />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '44px', marginTop: '48px' }}>
        ...
        {/* 전국 분포 */}
        <div>
          <h2 className="g-h2">한국 가구 순자산 분포 (2025년)</h2>
          <div className="tableScroll" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-card)' }}>
                ...
                {NATION_ROWS.map((r, i) => (
                  <tr key={i}>
                    <td style={{ ...td, fontWeight: 700 }}>
                      {r.k}
                      {!r.real && <span style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 500, marginLeft: 6 }}>추정</span>}
                    </td>
        ...
        {/* 연령대·지역 평균 */}
        <div>
          <h2 className="g-h2">연령대·지역별 평균 순자산</h2>
        ...
        {/* 순자산 분위별 자산 구성 */}
        <div>
          <h2 className="g-h2">순자산 분위별 자산 구성 — 위로 갈수록 부동산</h2>
          <div className="tableScroll" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-card)' }}>
        ...
        {/* 세계 분포 */}
        <div>
          <h2 className="g-h2">세계 순자산 분포 (UBS 2025)</h2>
          <div className="tableScroll" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-card)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
          ...
          <p style={{ fontSize: '12px', color: 'var(--muted)', margin: '12px 2px 0', lineHeight: 1.7 }}>
            UBS Global Wealth Report 2025(2024년 말 기준)의 구간 분포를 로그 보간하면 세계 상위 10% 진입선은 약 {usdK(W_TOP10_USD)}({usdToEok(W_TOP10_USD)}), 상위 1%는 약 {usdK(W_TOP1_USD)}({usdToEok(W_TOP1_USD)}) 수준입니다(추정 · 1달러 = {USD_KRW.toLocaleString('ko-KR')}원 환산).
          </p>
        </div>

        {/* 계산 방식 */}
        <div>
          <h2 className="g-h2">순위는 이렇게 계산합니다 — 분포 보간과 그룹 보정</h2>
          <p className="g-p">
            국가데이터처(옛 통계청) 조사는 모든 가구의 순자산을 공개하지 않고 중앙값·평균·분위별 평균·구간별 가구 비율 같은 요약값만 냅니다. 이 계산기는 조사 실측값(순자산 음수 가구 3.0%, 중앙값 2억 3,860만원, 3억 미만 57%, 10억 이상 11.8%)과 상위 구간 보도치(상위 5·1·0.1% 컷)를 백분위 축에 찍고, 그 사이 일부 점(하위 10~40%, 60%, 80% 등)은 평균 순자산·지니계수에 맞춘 추정점으로 채운 뒤, 점과 점 사이를 <strong>직선으로 이어(선형 보간)</strong> 입력한 순자산이 몇 번째 백분위에 해당하는지 구합니다. 기준점 사이 구간은 추정이므로 결과는 소수점까지 정확한 값이 아니라 대략적 위치로 읽어야 합니다.
          </p>
          <p className="g-p">
            시·도와 연령대는 해당 그룹의 분포 자료가 충분히 공개되지 않아, 전국 분포 모양이 같다고 가정하고 <strong>그룹 평균 ÷ 전국 평균</strong> 비율만큼 늘리거나 줄여 비교합니다. 예를 들어 서울은 평균 순자산이 {eokLabel(SEOUL.mean)}원으로 전국 평균의 {(SEOUL.mean / NATIONAL_MEAN).toFixed(2)}배이므로, 서울 가구의 {eokLabel(EX5.v)}원은 전국 기준 {eokLabel(Math.round(EX5.v / (SEOUL.mean / NATIONAL_MEAN)))}원과 같은 위치로 봅니다. 세계 순위는 UBS의 성인 1인 분포 구간을 로그 척도로 보간합니다.
          </p>
          <div className="tableScroll" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-card)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 460 }}>
              <thead>
                <tr>
                  <th scope="col" style={th}>순자산</th>
                  <th scope="col" style={{ ...th, textAlign: 'right' }}>전국 가구</th>
                  <th scope="col" style={{ ...th, textAlign: 'right' }}>서울 가구</th>
                  <th scope="col" style={{ ...th, textAlign: 'right' }}>39세 이하 가구</th>
                  <th scope="col" style={{ ...th, textAlign: 'right' }}>세계 성인 1인</th>
                </tr>
              </thead>
              <tbody>
                {RANK_ROWS.map((r) => (
                  <tr key={r.v}>
                    <td style={{ ...td, fontWeight: 700 }}>{eokLabel(r.v)}원</td>
                    <td style={tdNum}>상위 {r.nation.topPercent}%</td>
                    <td style={tdNum}>상위 {r.seoul.topPercent}%</td>
                    <td style={tdNum}>상위 {r.u39.topPercent}%</td>
                    <td style={tdNum}>상위 {r.world.topPercent}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* 순자산을 늘리는 일반 원칙 */}
        <div>
          <h2 className="g-h2">순자산을 늘리는 일반 원칙</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={card}>
              ...
              <p style={{ margin: 0, fontSize: '13px', color: 'var(--muted)', lineHeight: 1.8 }}>
                ...
                <Link href="/tools/finance/savings" style={{ color: 'var(--accent-ink)', fontWeight: 600 }}>저축액 계산기</Link>로 내 저축률을 한국 평균과 비교해 보세요.
              </p>
            </div>
            <div style={card}>
              ...
              <p style={{ margin: 0, fontSize: '13px', color: 'var(--muted)', lineHeight: 1.8 }}>
                ...
                <Link href="/tools/finance/loan" style={{ color: 'var(--accent-ink)', fontWeight: 600 }}>대출이자 계산기</Link>로 상환 방식별 이자 부담을 비교해 보세요.
              </p>
            </div>
            <div style={card}>
