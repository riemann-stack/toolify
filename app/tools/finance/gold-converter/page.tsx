...
const FAQ_LD = [
              ...
              },
              {
                ...
                a: '<strong>아닙니다. 헷갈리지 마세요!</strong><br/>• <strong>트로이온스 (oz t) = 31.1035g</strong> — 귀금속 전용 단위. 국제 금 거래 표준.<br/>• <strong>일반 온스 (avoirdupois oz) = 28.3495g</strong> — 식료품 등 일반 용도. 금에는 사용하지 않음.<br/><br/>국제 시세는 항상 트로이온스 기준(USD/oz t)으로 표시됩니다. 일반 온스로 환산하면 가격이 다르게 보이니 주의.',
              },
              {
              ...
              },
              {
                ...
                a: '네. <strong>개인이 골드바를 매수할 때 부가세 10%가 부과</strong>됩니다. 매수 가격은 시세 × 1.1(부가세)에 매장 수수료가 더해지는 구조라, 시세보다 10% 이상 비싸게 사게 됩니다.<br/><br/><strong>매도 시 부가세는 환급되지 않습니다.</strong> 즉, 매수 시 낸 부가세는 회수 불가. 따라서 시세가 부가세+스프레드를 상쇄할 만큼 상승해야 손익분기 — 단기 매매에 큰 함정.<br/><br/>예외 — <strong>KRX 금현물·금통장은 부가세 면제</strong> (단, 금통장 실물 인출 시 10% 추가 부과).',
              },
              {
              ...
              },
              {
                ...
                a: '네. <strong>금통장(KB·신한·우리·하나)의 매매차익은 배당소득으로 분류되어 15.4%(원천징수)</strong>가 부과됩니다. 또한 매도 차익이 2,000만원 초과 시 종합과세 합산.<br/><br/>예 — 1,000만원 투자 → 1,500만원 매도 → 차익 500만원 × 15.4% = 약 77만원 세금. 결국 423만원 수령. 같은 시세 차이를 KRX 금현물로 거래하면 비과세 — 500만원 그대로 수령.<br/><br/>금통장의 장점은 0.01g 단위 적립 가능 (소액 투자), 은행 직접 가입. 단점은 배당소득세 15.4% + 스프레드 1~2% + 실물 인출 시 부가세 10%.',
              },
              {
                q: '매수-매도 스프레드가 뭐고 왜 7~10%나 되나요?',
                a: '<strong>스프레드 = 매장이 사는 가격(매수가)과 파는 가격(매도가)의 차이</strong>. 매장의 마진과 위험 비용입니다.<br/><br/>예 — 매장이 시세보다 약 3~4% 높게 팔고 6~7% 낮게 되사면, 사는 값과 파는 값 사이에 약 10%의 스프레드가 생깁니다.<br/><br/>스프레드가 큰 이유 — 실물 골드바 보관·정련·재가공·시세 변동 대비 마진 확보 + 매장 운영비 + 부가세 마진. KRX 금현물은 디지털·중앙거래소라 0.3~0.5%로 압도적으로 낮습니다.',
              },
              {
                q: '코리아 프리미엄이란?',
                a: '<strong>한국 금 시세가 국제 시세보다 비싼 비율</strong>. 환율을 적용해 한국 1g 시세를 트로이온스(oz t)로 환산한 뒤 국제 시세(USD/oz)와 비교합니다.<br/><br/>정상 범위는 ±5%. 한국이 +5% 초과로 비싸면 「프리미엄」, -5% 미만으로 싸면 「디스카운트」라 부릅니다. 보통 환율 변동·국내 수급·세금 차이로 발생. 한국은 부가세 10%로 인해 골드바 가격 자체가 국제 시세 대비 +10% 베이스에서 시작.<br/><br/>본 도구의 [가격 계산] 탭에서 한국 시세 + USD/KRW 환율 + 국제 시세를 입력하면 자동 계산. 프리미엄/디스카운트 수준은 국내외 가격 괴리를 보는 참고 지표일 뿐이며, 매수·매도 판단의 근거로 삼기 전 수급·환율 요인을 함께 확인하세요.',
              },
            ]
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--bg2)' }}>
                    <td style={{ padding: '10px 12px', color: 'var(--accent)', fontWeight: 700, fontFamily: 'var(--font-sans)' }}>{unit}</td>
                    <td style={{ padding: '10px 12px', color: 'var(--text)', fontFamily: 'var(--font-sans)', fontWeight: 700 }}>{g}</td>
                    <td style={{ padding: '10px 12px', color: 'var(--muted)', fontFamily: 'var(--font-sans)' }}>{don}</td>
        ...
        <section>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {[
                  ['24K',  '100% (기준)', '999~999.9', '순금 기준점 — 골드바·돌반지·투자용. 실물 품위 99.9~99.99%. 부드러워 일상 착용 X'],
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--bg2)' }}>
                    <td style={{ padding: '10px 12px', color: 'var(--yellow-700)', fontFamily: 'var(--font-sans)', fontWeight: 800 }}>{k}</td>
                    <td style={{ padding: '10px 12px', color: 'var(--text)', fontFamily: 'var(--font-sans)', fontWeight: 700 }}>{ratio}</td>
                    <td style={{ padding: '10px 12px', color: 'var(--muted)', fontFamily: 'var(--font-sans)' }}>{mark}</td>
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--bg2)' }}>
                    ...
                    <td style={{ padding: '10px 12px', color: 'var(--emerald-600)' }}>{krx}</td>
                    <td style={{ padding: '10px 12px', color: 'var(--yellow-700)' }}>{bar}</td>
                    <td style={{ padding: '10px 12px', color: 'var(--cyan-600)' }}>{bank}</td>
                  </tr>
                ))}
              </tbody>
            </table>
