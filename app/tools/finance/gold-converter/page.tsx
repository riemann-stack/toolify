...
export const metadata = buildMetadata({
  ...
  title: '금 단위·시세·가격 계산기 — 돈 g 트로이온스 변환 + 14K/18K/24K 순도 환산 + KRX 금현물·골드바 비교 (2026년)',
  description:
    '한국 금 거래 종합 도구 — 돈·g·트로이온스·푼·냥 9단위 동시 변환, 14K/18K/22K/24K 순도 환산 매트릭스, 24K 시세 입력 시 매수·매도 실거래가 자동(부가세·스프레드·수수료·세공비), KRX 금현물 vs 골드바 vs 금통장 부가세·양도세 비교, 자산 가치 합산기, 코리아 프리미엄 계산.',
  keywords: [
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '60px 24px 80px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
        ...
        {/* 3. KRX 비교표 */}
        <section>
          <h2 style={sectionTitle}>KRX 금현물 vs 골드바 vs 금통장 — 부가세·양도세 비교</h2>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        {/* 5. 거래 비용 구조 */}
        <section>
          <h2 style={sectionTitle}>골드바 거래 비용 구조 (왜 단기 매매가 손해인가)</h2>
          <p style={faqAnswer}>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div>
              <p style={faqQuestion}>Q1. 1돈은 정확히 몇 그램인가요?</p>
              <div style={faqAnswer}>
                <strong style={{ color: 'var(--text)' }}>1돈 = 3.75g</strong>입니다. 한국 금 거래의 표준 단위로, 돌반지·골드바·돈치레 등 거의 모든 거래에 사용됩니다.
                <br /><br />
                관련 단위: 1푼 = 0.375g (1/10돈), 1냥 = 37.5g (10돈). 1kg = 266.67돈, 1트로이온스(oz t) = 31.1g ≈ 8.294돈.
              </div>
            </div>

            <div>
              <p style={faqQuestion}>Q2. 트로이온스(oz t)와 일반 온스(oz)는 같나요?</p>
              <div style={faqAnswer}>
                <strong style={{ color: '#FF8C8C' }}>아닙니다. 헷갈리지 마세요!</strong>
                <ul style={{ paddingLeft: '20px', margin: '8px 0' }}>
                  <li><strong style={{ color: 'var(--text)' }}>트로이온스 (oz t) = 31.1035g</strong> — 귀금속 전용 단위. 국제 금 거래 표준.</li>
                  <li><strong style={{ color: 'var(--text)' }}>일반 온스 (avoirdupois oz) = 28.3495g</strong> — 식료품 등 일반 용도. <strong style={{ color: '#FF8C8C' }}>금에는 사용하지 않음.</strong></li>
                </ul>
                국제 시세는 항상 트로이온스 기준 (예: $3,300/oz t). 일반 온스로 환산하면 가격이 다르게 보이니 주의.
              </div>
            </div>

            <div>
              <p style={faqQuestion}>Q3. 18K 반지를 녹이면 순금이 얼마나 나오나요?</p>
              <div style={faqAnswer}>
                18K = 75.0% 순금이므로 <strong style={{ color: 'var(--text)' }}>18K 10g → 순금 7.5g</strong> (24K 환산). 14K 10g → 순금 5.83g, 22K 10g → 순금 9.17g.
                <br /><br />
                실제 매도 시: 매장이 순금 환산 후 다시 스프레드(5~10%)와 정련 수수료(1~3%)를 차감하므로 18K 10g을 매도하면 순금 7.5g 시세의 87~92% 정도 받습니다. 본 도구의 &quot;가격 계산&quot; 탭에서 자동 시뮬 가능.
              </div>
            </div>

            <div>
              <p style={faqQuestion}>Q4. 골드바 살 때 정말 부가세 10%를 내나요?</p>
              <div style={faqAnswer}>
                네, <strong style={{ color: 'var(--text)' }}>개인이 골드바를 매수할 때 부가세 10%가 부과</strong>됩니다. 시세 1g = 14만원이면 매수 가격은 약 15.4만원 + 매장 수수료.
                <br /><br />
                <strong style={{ color: '#FF8C8C' }}>매도 시 부가세는 환급되지 않습니다.</strong> 즉, 매수 시 낸 부가세는 회수 불가. 따라서 시세가 부가세+스프레드를 상쇄할 만큼 상승해야 손익분기. 단기 매매에 큰 함정.
                <br /><br />
                예외: <strong style={{ color: 'var(--text)' }}>KRX 금현물·금통장은 부가세 면제</strong> (단, 금통장 실물 인출 시 10% 추가 부과).
              </div>
            </div>

            <div>
              <p style={faqQuestion}>Q5. KRX 금현물이 가장 유리한 이유?</p>
              <div style={faqAnswer}>
                3가지 이유:
                <ul style={{ paddingLeft: '20px', margin: '8px 0' }}>
                  <li><strong style={{ color: 'var(--text)' }}>부가세 면제</strong> — 매수 시 10% 절약</li>
                  <li><strong style={{ color: 'var(--text)' }}>양도소득세 비과세</strong> — 시세 차익에 세금 없음 (2026년 기준)</li>
                  <li><strong style={{ color: 'var(--text)' }}>매수-매도 스프레드 0.3~0.5%</strong> — 골드바(7~10%) 대비 압도적</li>
                </ul>
                단점: 실물 보유가 아닌 디지털 등록. 실물 전환 시 한국거래소 → 한국조폐공사 출고 절차 + 수수료. 증권사 계좌가 필요 (NH·키움·삼성·미래에셋 등 대부분 지원).
              </div>
            </div>

            <div>
              <p style={faqQuestion}>Q6. 금통장은 정말 양도세 15.4%를 내나요?</p>
              <div style={faqAnswer}>
                네. <strong style={{ color: 'var(--text)' }}>금통장(KB·신한·우리·하나)의 매매차익은 배당소득으로 분류되어 15.4%(원천징수)</strong>가 부과됩니다. 또한 매도 차익이 2,000만원 초과 시 종합과세 합산.
                <br /><br />
                예: 1,000만원 투자 → 1,500만원 매도 → 차익 500만원 × 15.4% = 약 77만원 세금. 결국 423만원 수령. 같은 시세 차이를 KRX 금현물로 거래하면 비과세 — 500만원 그대로 수령.
                <br /><br />
                금통장의 장점은 0.01g 단위 적립 가능 (소액 투자), 은행 직접 가입 (증권사 X). 단점은 양도세 + 스프레드 1~2% + 실물 인출 시 부가세 10%.
              </div>
            </div>

            <div>
              <p style={faqQuestion}>Q7. 매수-매도 스프레드가 뭐고 왜 7~10%나 되나요?</p>
              <div style={faqAnswer}>
                <strong style={{ color: 'var(--text)' }}>스프레드 = 매장이 사는 가격(매수가)과 파는 가격(매도가)의 차이</strong>. 매장의 마진과 위험 비용입니다.
                <br /><br />
                예: 시세 1g = 14만원이면 매장은 14.5만원에 판매(매도가)하고 13만원에 매입(매수가). 차이 1.5만원 = 약 10% 스프레드.
                <br /><br />
                스프레드가 큰 이유: 실물 골드바 보관·정련·재가공·시세 변동 대비 마진 확보 + 매장 운영비 + 부가세 마진. KRX 금현물은 디지털·중앙거래소라 0.3~0.5%로 압도적으로 낮음.
              </div>
            </div>

            <div>
              <p style={faqQuestion}>Q8. 코리아 프리미엄이란?</p>
              <div style={faqAnswer}>
                <strong style={{ color: 'var(--text)' }}>한국 금 시세가 국제 시세보다 비싼 비율</strong>. 환율을 적용해 한국 1g 시세를 트로이온스(oz t)로 환산한 뒤 국제 시세(USD/oz)와 비교합니다.
                <br /><br />
                정상 범위는 ±5%. 한국이 +5% 초과로 비싸면 &quot;프리미엄&quot;, -5% 미만으로 싸면 &quot;디스카운트&quot;라 부릅니다. 보통 환율 변동·국내 수급·세금 차이로 발생. 한국은 부가세 10%로 인해 골드바 가격 자체가 국제 시세 대비 +10% 베이스에서 시작.
                <br /><br />
                본 도구의 &quot;가격 계산&quot; 탭에서 한국 시세 + USD/KRW 환율 + 국제 시세를 입력하면 자동 계산. 프리미엄이 너무 높을 때(7%+)는 매수 자제, 낮을 때(-3%)는 매수 기회로 활용.
              </div>
            </div>
        ...
        <section>
          ...
          <ul style={{ paddingLeft: '20px', fontSize: '14px', color: 'var(--muted)', lineHeight: 2 }}>
            <li><Link href="/tools/finance/vat" style={{ color: 'var(--accent)' }}>부가세 계산기</Link> — 부가세 역산·견적서</li>
            <li><Link href="/tools/finance/compound" style={{ color: 'var(--accent)' }}>복리 계산기</Link> — 금 vs 예금·주식 장기 수익 비교</li>
            <li><Link href="/tools/finance/stock" style={{ color: 'var(--accent)' }}>주식 물타기 계산기</Link> — 평단가 시뮬</li>
            <li><Link href="/tools/finance/savings" style={{ color: 'var(--accent)' }}>월 저축가능 금액 계산기</Link> — 금 적립 시뮬 시</li>
            <li><Link href="/tools/unit/converter" style={{ color: 'var(--accent)' }}>단위 변환기</Link> — 길이·부피·온도 등 일반 단위</li>
          </ul>
        </section>
