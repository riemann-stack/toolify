...
const FAQ_LD = [
              ...
              },
              {
                ...
                a: '2021년 7월부터 간이과세자도 연 매출 4,800만 원 이상이면 세금계산서 발급이 의무화되었습니다. 반면 직전 연도 공급대가가 4,800만 원 미만인 간이과세자와 신규 간이과세자는 세금계산서를 발급할 수 없고 영수증(신용카드 매출전표·현금영수증 포함)만 발급합니다. 거래처가 매입세액 공제용 세금계산서를 꼭 받아야 한다면 일반과세 전환을 검토해야 합니다.',
              },
              {
              ...
              },
              {
                ...
                a: '본 도구의 "실입금 역산" 탭에서 사업자 유형·플랫폼별로 자동 계산(부가세는 통과 자금이라 실수입에서 제외). 사업자 (직거래, 부가세 별도): 110만, 사업자 (크몽 20%): 약 141만, 사업자 (네이버페이 2.5%): 약 113.1만, 프리랜서 (직거래): 약 103.4만, 프리랜서 + 크몽: 약 130.4만. ⚠️ 부가세는 본인 돈이 아닙니다 — 별도 계좌에 보관 후 신고·납부 권장.',
              },
              {
                q: '일반과세자 vs 간이과세자 어느 게 유리한가요?',
                a: '본 도구의 "일반 vs 간이" 탭에서 자동 추천. 일반적으로 매입이 많은 (40%+) B2B는 일반과세 (매입세액 공제 큼), 매입이 적은 B2C는 간이과세 (실효세율 1.5~4%)가 유리합니다. 연 매출(공급대가) 1억 400만 원 이상이면 간이 자격이 없고, 부동산임대업은 4,800만 원 이상이면 간이과세를 적용받을 수 없습니다. 간이과세자는 연 매출 4,800만 원 미만이면 부가세 납부가 면제됩니다(신고는 필요). 거래처가 세금계산서를 요구하면 일반과세 권장. 2024년 한도 상향(8천 → 1억 400만)으로 소규모 사업자에게 간이가 더 유리해진 환경입니다.',
              },
              {
                q: '사업자 등록 안 한 프리랜서도 부가세 신고하나요?',
                a: '사업자 등록 X = 부가세 신고 의무 X. 대신 3.3% 원천세(소득세 3% + 지방세 0.3%)가 적용되며, 발주처가 차감 후 입금합니다. 종합소득세 신고 시 정산. 사업자 등록을 고려해야 할 시점: 사업으로 계속·반복해 매출이 생길 때(매출이 작으면 간이과세로 시작할 수 있고, 연 매출 4,800만 원 미만 간이과세자는 부가세 납부가 면제됩니다), 거래처가 세금계산서를 요구할 때, 매입세액 공제를 받고 싶을 때, 사업 규모를 키울 때. 사업자 등록 후 부가세 신고 의무 발생.',
              },
              {
        ...
        <Section>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid rgba(8,145,178,0.2)', borderRadius: 'var(--radius-m)', padding: '16px 20px' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                ...
                <tbody>
                  {[
                    ...
                    ['제조·농업·임업·어업',             '20%', '2.0%'],
                    ['숙박업',                          '25%', '2.5%'],
                    ['건설·운수·정보통신·기타서비스',   '30%', '3.0%'],
                    ['금융보험·전문·부동산임대',        '40%', '4.0%'],
                  ].map(([biz, rate, eff], i) => (
            ...
            <p style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '8px' }}>
              ...
              매입세액공제는 2021년 7월부터 <strong style={{ color: 'var(--text)' }}>세금계산서 수취 공급대가의 0.5%</strong>로 제한됩니다(업종별 부가가치율 곱 방식 폐지).
            </p>
          </div>
        </Section>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {[
                  ...
                  ['프리랜서 + 네이버페이 (2.5%)',       '약 106.2만'],
                  ['프리랜서 + 크몽 (20%)',             '약 130.4만'],
                  ['사업자 (직거래, 부가세 별도)',       '110만'],
                  ['사업자 + 네이버페이 (부가세 별도)',  '약 113.1만'],
                  ['사업자 + 크몽 (부가세 별도)',        '약 141.0만'],
                ].map((row, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--bg2)' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {[
                  ...
                  ['간이과세자 (4,800만↓·신규)', '발급 불가 (영수증만)',            '신용카드 전표·현금영수증'],
