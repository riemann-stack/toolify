      ...
      <svg viewBox={`0 0 ${W} ${H}`} className={styles.chartSvg} preserveAspectRatio="xMidYMid meet">
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((t) => (
          <g key={t}>
            ...
            <text x={padL - 8} y={padT + (H - padT - padB) * (1 - t) + 5}
              fill="var(--muted)" fontSize="15" textAnchor="end" fontWeight="600">
              {fmtKRW(maxV * t)}
            </text>
          </g>
        ))}
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((t) => (
          <text key={t} x={padL + (W - padL - padR) * t} y={H - 16}
            fill="var(--muted)" fontSize="15" textAnchor="middle" fontWeight="600">
  ...
  return (
    <div className={styles.chartWrap}>
      <svg viewBox={`0 0 ${W} ${H}`} className={styles.chartSvg} preserveAspectRatio="xMidYMid meet">
        ...
        <text x={padL - 8} y={yOf(initial) + 5} fill="var(--muted)" fontSize="15" textAnchor="end" fontWeight="600">
          {fmtKRW(initial)}
        </text>
        <text x={padL - 8} y={yOf(maxV) + 5} fill="var(--muted)" fontSize="15" textAnchor="end" fontWeight="600">
          {fmtKRW(maxV)}
        </text>
        {/* x-axis labels */}
        {[0, 0.5, 1].map((t) => (
