...
const FAQ_LD = [
              ...
              },
              {
                ...
                a: '본 계산기는 2026년 4대보험 요율을 적용하고, 소득세는 근로소득 간이세액표를 칸 단위로 찾지 않고 연 결정세액을 12로 나눠 근사한 추정입니다. 실제 차이가 발생하는 원인 — 간이세액표 칸 값과 근사치의 차이 / 회사별 비과세 항목(식대·교통비·복지) / 상여금·성과급(지급 시점 따라 변동) / 두루누리 사회보험 지원(소규모 사업장) / 부양가족 변동(출생·결혼) / 추가 공제(의료비·기부금 등). 정확한 금액은 회사 인사팀 또는 급여명세서를 확인하세요. 본 도구는 ±2~5% 오차 범위의 추정치입니다.',
              },
              {
                q: '월 실수령액 역산은 어떻게 활용하나요?',
                a: '<strong>[역산] 탭</strong>에서 원하는 월 실수령액(예: 300만원)을 입력하면 필요한 세전 연봉(약 4,210만원)이 자동 계산됩니다. 이직·연봉 협상 시 매우 유용합니다 — 본인이 원하는 생활비 수준 → 필요 연봉 → 협상 시작 금액 결정. 본 도구는 협상 안정 범위(원하는 연봉의 +1~6%)도 함께 제시합니다.',
              },
              {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'center', color: '#EA580C',       fontWeight: 500, whiteSpace: 'nowrap' }}>변경</th>
                </tr>
              </thead>
              <tbody>
                {[
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
            {[
              ...
              <div key={i} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <span style={{ fontSize: '11px', color: 'var(--accent)', background: 'var(--accent-dim)', borderRadius: 'var(--radius-pill)', padding: '2px 8px', alignSelf: 'flex-start' }}>{item.limit}</span>
        ...
        <section>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'center', color: 'var(--accent)', fontWeight: 700 }}>세율</th>
          ...
          <p style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '10px', lineHeight: 1.7 }}>
            ⓘ 본 도구의 소득세는 간이세액표 근사(연 결정세액÷12) 추정 — 실제 연말정산은 의료비·기부금·신용카드 등 추가 공제로 달라집니다.
          </p>
        </section>
