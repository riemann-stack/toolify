...
const FAQ_LD = [
  { "q":"3.3% 원천징수만 내면 되는 거 아닌가요?","a":"아닙니다. 3.3% (소득세 3% + 지방소득세 0.3%)는 임시로 미리 낸 세금일 뿐, 진짜 세금은 매년 5월 종합소득세 신고로 정산합니다. 본인의 실제 세율(6~45% 누진)에 따라 더 받거나 더 내야 합니다. 매출이 적고 경비·공제가 충분하면 환급, 매출이 크고 한계세율 24% 이상이면 추가 납부가 일반적입니다." },
  { "q":"단순경비율과 장부 작성, 어느 게 유리한가요?","a":"실제 경비가 단순경비율보다 크면 장부, 적으면 단순경비율이 유리합니다. 예: IT 개발자 매출 5천만원 → 단순경비율 64.1%로 3,205만 경비 자동 인정. 본인이 실제로 사무실·장비·소프트웨어에 4천만 썼다면 장부 작성이 800만 차이만큼 유리. 다만 장부 작성에는 증빙(영수증·세금계산서·신용카드) 보관과 매출/경비 기록 의무가 있습니다. 매출 7,500만(인적용역 기준) 이상이면 단순경비율 적용 자체가 불가능하니 장부가 필수." },
  { "q":"노란우산공제 vs 연금저축, 뭐가 절세 효과가 큰가요?","a":"본인 한계세율에 따라 다릅니다. 노란우산은 소득공제 → 한계세율만큼 절세. 한계세율 24%면 200만 납입 → 약 53만 절세 (지방세 포함). 연금저축은 세액공제 → 종합소득금액 4,500만 이하 16.5%, 초과 13.2% 정액. 600만 납입 → 79~99만 정액 절세. 과세표준 5천만 이상(한계세율 24%+)이면 노란우산이, 4,500만 이하 저소득 구간이면 연금저축의 16.5% 정률이 유리합니다. 본 도구의 \"시나리오 비교\" 탭에서 즉시 확인 가능." },
  { "q":"직장인 + 부업 프리랜서, 합산 신고 어떻게 하나요?","a":"근로소득과 사업소득은 합산되어 종합과세됩니다. 회사에서 연말정산을 마쳐도, 부업 사업소득이 있으면 5월에 종소세 신고 의무가 있습니다. 합산 시 한계세율이 더 높은 구간으로 점프해 추가 납부가 발생하는 경우가 흔합니다. 예: 본업 7,000만(24% 구간) + 부업 1,500만 → 합산 8,500만으로 24% 구간 경계, 부업 일부는 35% 적용 가능. 본 계산기는 단독 사업소득 기준이므로 겸업자는 본업 근로소득과 합산해 계산해야 정확." },
  { "q":"신고 안 하면 어떻게 되나요?","a":"무신고 시 다음 가산세가 부과됩니다: 무신고 가산세 20% (부정 무신고 40%) 납부지연 가산세 일 0.025% (연 약 9.1%) 무기장 가산세 20% (복식부기 의무자가 미작성 시) 또한 환급 대상이었어도 신고하지 않으면 환급 받지 못합니다. 5월 1~31일 신고 의무는 매출 규모와 무관하게 모든 사업소득자에게 적용됩니다." },
  { "q":"종소세 신고하면 건강보험료가 오른다는데?","a":"직장가입자가 아닌 지역가입자의 경우, 종합과세된 사업소득이 다음 해 11월부터 건보료 산정 기준에 반영됩니다. 매출이 크게 늘어난 해의 다음 해 건보료가 인상되는 패턴입니다. 다만 경비 처리·공제로 사업소득금액 자체가 줄어들면 건보료도 함께 줄어듭니다. 종소세 절세 = 건보료 절세로 이어지는 효과. 직장가입자는 본업 근로소득 기준이므로 부업 사업소득이 일정 기준 미만이면 영향 없음." },
  { "q":"장부 작성 의무는 언제부터 생기나요?","a":"업종별 직전년도 매출 기준: 간편장부: 일반 (사업 시작 후 누구나 권장) 복식부기 의무: 인적용역 7,500만 / 부동산임대 7,500만 / 음식·숙박 1억5천 / 도소매 3억 등 복식부기 의무자가 미작성하면 무기장 가산세 20%, 단순경비율 적용 불가. 매출이 위 한도를 넘으면 세무사 또는 회계 프로그램(삼쩜삼·자비스 등)을 통해 복식부기 작성이 일반적입니다." },
  { "q":"수입 1억 넘으면 세무사 써야 하나요?","a":"의무는 아니지만 실익이 큰 시점입니다: 한계세율 35% 이상 → 절세 1만원 = 세무사비용 회수 빠름 복식부기 의무 → 정확한 장부 + 결산 보고서 필요 경비 항목별 한도·증빙 요건 복잡 → 누락 시 큰 손실 사업자등록(개인사업자/법인) 전환 절세 검토 필요 매출 1.5억 이상이면 거의 필수. 매출 5천~1억 구간은 본 도구로 자가진단 → 세무사 1회 자문 → 다음 해부터 직접 신고 흐름이 일반적입니다. 5월 종합소득세 신고 절차 요약 1~3월: 거래처에서 지급명세서 발급 (1월 말까지) · 1년 매출/경비 정리 4월: 노란우산·연금저축 납입증명서 수령 · 부양가족 자료 점검 5월 1~31일: 홈택스 종소세 신고 (모두채움 또는 일반신고) 5월 31일: 납부 마감 (추가 납부 시) — 자정까지 인터넷지로 6~7월: 환급액 입력 계좌로 자동 입금 11월: 다음 해 지역 건강보험료 변동 (사업소득 반영) {/* 6." }
]
