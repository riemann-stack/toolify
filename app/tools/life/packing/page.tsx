...
export const metadata = buildMetadata({
  ...
  title: '여행 옷·짐 계산기 — 일수·기온·세탁 → 카테고리별 개수 + 체크리스트',
    ...
    <div style={{ maxWidth: '880px', margin: '0 auto', padding: '60px 24px 80px' }}>
      ...
      <div style={card}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}>
          {[
            { t: '❄️ 혹한 (-10°C↓)', d: '두꺼운 니트·플리스·기모. 롱패딩·롱코트. 내복·핫팩·털모자·장갑·머플러 필수.', c: '#3EC8FF' },
            { t: '🥶 겨울 (0~10°C)', d: '니트·후리스·긴팔. 패딩·트렌치. 머플러·장갑.', c: '#9B59B6' },
            { t: '🍂 봄·가을 (10~20°C)', d: '긴팔·맨투맨·셔츠. 자켓·가디건·바람막이. 일교차 큼 → 레이어드.', c: '#FFB83E' },
            { t: '🌤️ 초여름 (20~25°C)', d: '반팔·긴팔 혼용. 가벼운 가디건 (실내 냉방). 우산·선글라스.', c: '#3EFFD0' },
            { t: '☀️ 여름 (25~30°C)', d: '반팔·민소매·린넨. 모자·선글라스·선크림 SPF 50+.', c: '#FF8C3E' },
            { t: '🔥 한여름 (30°C↑)', d: '통풍 셔츠·드라이핏·민소매. 수영복 2벌. 휴대용 선풍기.', c: '#FF3E8C' },
          ].map((g, i) => (
                ...
                <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                  {row.map((cell, j) => (
                    <td key={j} style={{
                      ...
                      fontFamily: j === 0 || j === 3 ? 'Noto Sans KR, sans-serif' : 'Syne, sans-serif',
