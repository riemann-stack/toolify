...
const FAQ_LD = [
  { "q":"미국 식당 팁은 얼마가 적당한가요?","a":"15~20%가 표준이며, 만족도에 따라 조정합니다: • 보통 (Standard): 15% • 좋음 (Good): 18% • 매우 좋음 (Excellent): 20% 또는 그 이상 • 고급 레스토랑은 20-25% • 카운터·테이크아웃은 10% 또는 잔돈 ⚠️ 단체 6명+는 자동 18% 팁(Auto Gratuity)이 청구되니 영수증 확인. 기준은 세전 금액(subtotal)이 일반적이지만, 세후 기준도 점차 확산." },
  { "q":"일본에서 팁을 주면 안 되나요?","a":"🚫 주지 마세요. 일본은 팁 문화가 없으며, 오히려 무례하게 받아들일 수 있습니다. 가격에 모든 서비스가 포함되어 있다는 자부심. 예외: • 료칸(旅館): 心付け(고코로즈케) ¥1,000~3,000을 작은 봉투(ぽち袋)에 • 고급 호텔 컨시어지: 특별한 부탁 시 봉투에 현금을 그대로 건네면 거절당할 수 있어요. 정 감사 표현하고 싶다면 한국식 선물(떡·과자·차)이 더 자연스럽습니다." },
  { "q":"봉사료(Service Charge)가 이미 포함된 경우?","a":"이미 봉사료가 자동 청구된 경우 추가 팁은 선택입니다. 영수증 확인 항목: • \"Service Charge\"·\"Gratuity\": 봉사료 (보통 10-12.5%) • \"Auto Gratuity\": 자동 팁 (미국 단체) • \"Service Compris\" (프랑스): 봉사료 포함 • \"含む\"·\"包含\" (일본·중국): 포함 이미 포함되어 있다면 잔돈 반올림이나 €1~2 추가 정도가 매너. 두 번 내지 마세요." },
  { "q":"카드로 결제하면 팁은 어떻게?","a":"국가·매장별로 다릅니다: • 미국·캐나다: 영수증에 \"Tip\" 빈칸 → 직접 적고 사인 • 독일·북유럽: 카드 단말기 입력 시 미리 말해야 함 (\"30 Euro\") • 일본: 카드 결제 시 팁 X (애초에 팁 X) • 동남아·발리: 카드도 가능하지만 현금이 더 확실 (직원 손에 직접) ⚠️ 벨보이·룸메이드·기사·캐디는 항상 현금이 표준 — 카드로 줄 수 없음. 여행 전 $1·$5·£1·€1 등 소액 지폐 충분히 환전이 핵심." },
  { "q":"단체 식사에 자동 팁이 붙는 이유?","a":"미국·캐나다는 6명 이상 단체 식당 이용 시 자동 18% 팁(Auto Gratuity)이 청구됩니다. 이유: • 단체는 결제·계산이 복잡해 직원 부담 ↑ • 일행 중 한 명이 팁을 안 주면 분배 어려움 • 매장 정책으로 일관성 확보 영수증 하단에 \"Gratuity 18% included\" 표시. 또 팁을 추가하지 마세요 — 이중 팁이 됩니다. 서비스가 매우 좋았다면 추가 2~5%만 권장." },
  { "q":"호텔 벨보이·룸메이드 팁 얼마?","a":"현금 + 정액제가 원칙: • 벨보이 (Bellboy): 짐 1개당 $1~2 (5성급은 $2~3) • 도어맨 (Doorman): 택시 잡아주면 $1~2 • 룸메이드 (Housekeeper): 매일 아침 베개 위 $2~5 (체크아웃 시 한 번에 X — 매일 사람이 다름) • 컨시어지 (Concierge): 특별 예약·티켓 도움 시 $5~20 • 룸서비스: 영수증에 이미 18% 포함 多 → 확인 후 추가 유럽은 약 절반 (€1~2). 동남아는 호텔 등급별 다름 (5성급은 미국 수준)." },
  { "q":"우버·택시는 팁 필수인가?","a":"• 🇺🇸 미국·🇨🇦 캐나다: 15~20%가 관례 (거의 기대되지만, 우버는 공식적으로 선택 사항). 우버 앱에서 도착 후 입력 가능 (잊지 말 것) • 🇪🇺 유럽: 잔돈 반올림 (€10.50 → €11 등) 또는 5% • 🇯🇵 일본: X (오히려 거절) • 🇸🇬 싱가포르·🇭🇰 홍콩: X 또는 잔돈 • 🇹🇭 태국·🇻🇳 베트남: 미터 사용 시 잔돈만, 흥정 가격엔 별도 X 짐을 트렁크에 실어주거나 특별 도움 시 추가 $1~5." },
  { "q":"동남아 마사지 팁은 얼마?","a":"한국 관광객에게 가장 인기 있는 카테고리입니다 (가성비 최고). • 🇹🇭 태국: 50~100바트 (1시간 마사지 기준). 1회 200바트 정도 = 매우 만족 • 🇮🇩 발리: Rp 10,000~50,000 • 🇵🇭 필리핀: 50~100페소 • 🇻🇳 베트남: VND 20,000~50,000 ※ 5성급 호텔 스파는 비율(15~20%) 적용. 로컬 마사지샵은 정액 권장. 마사지사 손에 직접 (사장님 X). 봉투에 넣어주면 더 좋음." },
  { "q":"골프 캐디 팁 (동남아)?","a":"한국 골프투어객의 핵심 페인 포인트입니다. • 🇵🇭 필리핀 (클락·세부): 캐디 200~500페소, 캐디 마스터 100페소 • 🇹🇭 태국: 캐디 500~800바트 (1라운드) • 🇻🇳 베트남 (다낭·하노이): 캐디 VND 200,000~500,000 • 🇮🇩 인도네시아: 캐디 Rp 100,000~200,000 ※ 현금 필수 (페소·바트 등 현지 통화). 만족도가 매우 좋으면 1.5~2배. 단체 라운드면 캐디·캐디 마스터·골프장 도우미·기사 모두 별도 준비." },
  { "q":"팁을 안 주면 어떻게 되나요?","a":"국가별로 결과가 매우 다릅니다: • 🇺🇸 미국: 큰 결례. 직원이 따라 나와 항의할 수도 (인종차별로 오해될 위험). 다음 방문 시 서비스 X • 🇪🇺 유럽: 무례하게 보이지만 큰 문제 X • 🇯🇵 일본·🇨🇳 중국: 오히려 안 주는 게 정상 • 🇹🇭 동남아: 서운해 하지만 강제 X 미국은 직원 시급이 매우 낮고 (팁 포함 시급) 팁이 소득의 큰 비중(팁 크레딧 제도) — 다만 미 노동부(DOL) 기준상 팁이 최저임금에 못 미치면 고용주가 차액을 보전해야 합니다. 그래서 미국·캐나다 식당·우버는 예산에 팁을 미리 포함해 계산하는 것이 마음 편합니다." }
]
  ...
  return (
    <div style={{ maxWidth: '880px', margin: '0 auto', padding: '60px 24px 80px' }}>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          • <strong>🇺🇸 미국·🇨🇦 캐나다</strong>: <strong>15~20%가 관례</strong> (거의 기대되지만 우버는 공식적으로 선택). 앱에서 도착 후 입력 가능 (잊지 말 것)<br />
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          미국은 직원 시급이 매우 낮고 (팁 포함 시급) 팁이 소득의 큰 비중(팁 크레딧 제도) — 다만 미 노동부(DOL) 기준상 팁이 최저임금에 못 미치면 고용주가 차액을 보전해야 합니다.
