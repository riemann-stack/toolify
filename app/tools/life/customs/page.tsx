...
const FAQ_LD = [
  ...
  { "q":"알리·테무는 왜 거의 무세금?","a":"중국 직구 면세 한도 $150 = 약 21만원. 알리·테무 대부분 상품이 저가($5~30)라 한 건당 면세 한도에 한참 못 미칩니다. • 1건 주문 = 보통 $20~50 = 면세 • 단 2일 내 여러 건 주문 시 합산으로 과세 위험 • 가구·가전 등 고가는 과세 (예: 청소기 $200 → 과세) ⚠️ 최근 알리·테무 직구 폭증으로 관세청이 통관 강화 중 — 분할 주문 권장." },
  { "q":"명품 가방 직구 vs 한국 면세점?","a":"유럽 명품 가방 €1,500 (약 225만원) 기준: • 직구 (Matchesfashion): 225만 + 관세 8% (18만) + 부가세 (24.3만) + 개소세 (5만, 200만 초과분) = 약 272만원 • 한국 백화점: 약 350~400만원 • 한국 면세점: 약 240~280만원 → 직구가 백화점 대비 30% 절감, 면세점과 비슷. 단 A/S·정품 인증은 백화점이 우월. 명품 직구는 정식 매장 영수증 보관 필수." },
  { "q":"노트북·핸드폰은 정말 무관세?","a":"네, 관세 0%입니다 (HS Code 8471·8517 — 정보기술협정 ITA). 단 부가세 10%는 면제 X. 미국 면세 한도 $200을 초과하면: • 노트북 $1,500 (약 210만) → 관세 0 + 부가세 21만 = 약 231만원 • 한국 출시가 대비 보통 20~30% 절감 (애플·아수스·델 등) • A/S는 한국 공식 서비스센터에서 가능 (글로벌 워런티) 단 노트북 + 다른 품목 같이 주문은 합산 위험 — 노트북만 따로 주문 권장." },
  { "q":"합산 과세란? (2일 내 같은 사람)","a":"같은 사람(수취인)·같은 발송지·2일 이내 도착하는 직구는 합산되어 면세 한도가 한 번만 적용됩니다. 예시: • 같은 날 아마존에서 $150 + $80 주문 → 합산 $230 → 과세 • 1주일 시간차 주문 → 도착일 분리 → 각각 면세 기준은 국내 도착일이며 배대지·국제배송 일정에 따라 변동. 안전하게 1주일 간격 권장. 관세청 통관시스템에서 자동 합산되므로 회피하기 어렵습니다." },
  { "q":"분할 발송으로 면세 가능?","a":"가능하지만 주의 사항 多: ✅ 안전한 분할: • 1주일 이상 시간차로 주문 • 다른 사이트·다른 발송지 활용 (eBay + Amazon) • 가족 명의로 분산 (단, 명의 도용은 불법) ❌ 위험한 분할: • 같은 사이트에 같은 날 분할 주문 • 의도적 회피로 판단되면 관세 회피로 처벌 가능 • 통관 시스템이 자동 합산 — 적발 위험 ↑ ※ 면세 한도 회피보다는 한도 내 알찬 직구가 정답." },
  ...
  { "q":"통관 수수료는 얼마?","a":"• 목록통관 (면세): 통관 수수료 X (관세사 안 거침) • 일반통관 (과세): - 관세사 수수료 약 5,000~30,000원/건 (가격·품목별) - 큰 화물은 추가 - EMS·DHL·FedEx는 자체 통관 (포함된 경우 多) - 배대지(Shipster·Mall) 사용 시 별도 수수료 (kg당 5천~1만원) 본 도구의 계산은 통관 수수료 미포함 — 실제 결제 시 +5천~3만원 추가 예상." },
