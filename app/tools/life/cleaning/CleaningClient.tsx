...
function recipe(sit: (typeof SITUATIONS)[number], volumeMl: number) {
  ...
  if (sit.gPerL && sit.gPerL > 0) {
    ...
    return { amount: `${g.toLocaleString('ko-KR')}g`, sub: tbsp ? `약 ${tbsp}큰술` : '' }
  }
  ...
  return { amount: '사용법 참고', sub: '용량과 무관 (정량 사용)' }
}
    ...
    <div className={s.wrap}>
      <Disclaimer
        ...
        related={[
          ...
          { href: '/tools/cooking/storage', label: '식재료 보관' },
      ...
      <div className={s.card}>
        ...
        <div className={s.sitGrid}>
          {SITUATIONS.map((x) => (
            ...
            >
              <span className={s.sitEmoji}>{x.emoji}</span>
      ...
      <div className={s.result}>
        ...
        <div className={s.recipeBox}>
          ...
          <div className={s.recipeMain}>
            물 <strong>{volume.toLocaleString('ko-KR')}ml</strong>
            {r.amount !== '사용법 참고' && <> + {agent.name} <strong style={{ color: agent.color }}>{r.amount}</strong></>}
          </div>
