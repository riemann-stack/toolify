...
const FAQ_LD = [
  { "q":"fetch와 axios 어떤 게 더 좋나요?","a":"상황에 따라 다릅니다. fetch (권장): • 브라우저·Node 18+ 내장 (의존성 0) • 표준 Web API · 단순 • 단점: JSON 자동 변환 X (.json() 호출 필요), 인터셉터 X, 취소는 AbortController axios: • 인터셉터 (요청·응답 변환)·자동 JSON·진행률·취소 토큰·재시도 등 풍부한 기능 • 구버전 브라우저(IE) 지원 • 단점: npm i axios 의존성, 번들 크기 ↑ 현대 프로젝트 (Next.js·Vite): fetch + 필요시 SWR/TanStack Query 조합 권장. axios는 인터셉터·재시도가 핵심일 때." },
  { "q":"Python에선 requests vs httpx 차이?","a":"requests는 가장 널리 쓰이는 HTTP 클라이언트(2011~)로 동기(sync)만 지원하며 단순·직관적이고 문서가 풍부합니다. httpx는 requests 호환 API에 동기·비동기를 모두 지원하고 HTTP/2를 기본 지원해 FastAPI 등 비동기 프로젝트에 적합합니다. 본 도구는 가장 일반적인 requests 코드를 생성합니다. httpx 사용자는 import 부분만 import httpx로 변경하면 거의 호환되며, 비동기는 httpx.AsyncClient()를 사용합니다." },
  { "q":"Node.js fetch는 언제부터?","a":"Node.js 18(2022.04)부터 fetch가 전역으로 사용 가능합니다(node-fetch 패키지 불필요). Node 18~20은 플래그 없이 쓸 수 있는 실험적 단계, Node 21 이상은 안정(stable), Node 16 이하는 npm i node-fetch가 필요합니다. 본 도구의 fetch 코드는 Node 18 이상을 가정하며, 구버전이면 import fetch from 'node-fetch'를 추가하거나 axios·node http 코드를 사용하세요." },
  { "q":"다중 라인 cURL (\\) 지원하나요?","a":"네, 지원합니다. Chrome/Firefox DevTools의 \"Copy as cURL\"이 만드는 다중 라인 형식을 그대로 붙여넣으면 됩니다. 백슬래시+개행(셸 표준), 백슬래시+CRLF(Windows), 한 줄 형식을 읽고, 본문에 작은따옴표·느낌표·줄바꿈이 있을 때 Chrome이 쓰는 $'…' 형식도 풀어서 해석합니다. 다만 셸을 완전히 흉내 내지는 않으므로 변수 치환($VAR)이나 명령 치환은 처리하지 않습니다." },
  { "q":"파일 업로드(-F)는 변환되나요?","a":"multipart 구조는 지원하며 파일 자체는 placeholder로 생성됩니다. -F \"file=@photo.jpg\"는 fetch/axios에서 FormData에 파일을 추가하는 형식(브라우저 파일 입력 가정)으로, Python에서는 open('photo.jpg', 'rb')가 자동 추가되고, Node.js는 form-data 패키지(fs.createReadStream), Go는 mime/multipart와 os.Open으로 파일을 읽는 코드가 만들어집니다. 파일 경로와 브라우저 파일 입력은 환경에 맞게 바꿔야 하며, 비-multipart 파일(-d @body.txt)은 미지원으로 경고가 표시됩니다." },
  { "q":"Authorization 토큰을 안전하게 처리하려면?","a":"핵심 원칙은 토큰을 코드에 직접 하드코딩하지 않고 환경 변수로 분리하는 것입니다. 브라우저 JavaScript에는 절대 넣지 말고 백엔드 프록시를 경유하세요. Node.js는 process.env.API_TOKEN과 .env 파일(.gitignore 등록), Python은 os.environ 조회와 python-dotenv, Go는 os.Getenv와 .env를 사용합니다. CI/CD는 GitHub Actions Secrets·GitLab CI Variables, 운영 환경은 AWS Secrets Manager·Vault·Doppler 등 시크릿 매니저를 권장합니다. 본 도구의 마스킹 옵션을 켜면 코드 출력 시 ***로 표시되어 우연한 노출을 방지합니다." },
  { "q":"multipart/form-data 변환?","a":"-F 옵션이 multipart로 자동 변환됩니다. • fetch / axios: FormData 객체 생성. Content-Type 헤더 직접 설정 금지 (boundary 자동 생성) • Python: files= + data= 분리 • Node.js: form-data 패키지로 본문 생성 • Go: mime/multipart Writer로 본문 생성 주의: cURL의 -F는 자동으로 Content-Type: multipart/form-data; boundary=...를 생성합니다. 사용자가 직접 -H로 Content-Type을 지정하면 boundary 누락으로 깨질 수 있어요. 본 도구는 multipart 본문일 때 입력한 Content-Type 헤더를 5개 언어 코드 모두에서 빼고, 각 라이브러리가 boundary를 붙이게 합니다." },
  { "q":"GraphQL cURL 변환?","a":"GraphQL은 일반 HTTP POST + JSON body이므로 완벽히 변환됩니다. query·variables 필드가 포함된 JSON 본문이 5개 언어 코드로 그대로 변환됩니다. 본 도구의 탭 3 예시에서 GraphQL 카드를 적용해 보세요. 더 정교한 GraphQL 클라이언트(Apollo·urql·graphql-request)는 별도 라이브러리 사용을 권장합니다." },
  { "q":"AWS S3 cURL 변환?","a":"구조는 변환되지만, AWS Signature V4 서명은 사용자가 직접 처리해야 합니다. • 본 도구: cURL의 헤더(Authorization V4·x-amz-content-sha256·x-amz-date)를 그대로 5 언어 코드로 옮김 • 실제 운영: - JavaScript: @aws-sdk/client-s3 사용 (서명 자동) - Python: boto3 - Go: aws-sdk-go-v2 본 도구는 학습·디버깅용이며 운영 환경에서는 SDK 사용 권장. 본 도구의 [📚 예시] 탭의 AWS S3 카드는 V4 서명 헤더 형식 참고용입니다." },
  { "q":"본 도구는 입력 데이터를 서버에 보내나요?","a":"아니요. 모든 처리가 브라우저(클라이언트)에서 수행됩니다. • cURL 파싱: 직접 작성한 토큰화 + 플래그 파서 (~500줄, 외부 라이브러리 0) • 코드 생성: 템플릿 문자열 (서버 호출 0) • Network 탭 확인: 변환 시 어떤 fetch/XHR도 발생하지 않음 • 입력은 localStorage에만 저장 (재방문 편의), 외부 전송 X. Authorization·Cookie·API 키 헤더나 -u 인증이 든 입력은 저장하지 않고 옵션만 저장합니다. 다만 공용 PC·공유 기기에서 토큰을 다뤘다면 사용 후 정리하세요. DevTools → Application → Local Storage에서 youtil_curl_v1 키 삭제 가능. 민감 토큰은 마스킹 옵션으로 코드에서 *** 처리하고, 실제 값은 환경 변수로 옮기는 것을 권장합니다." }
]
    ...
    <ToolPage width={880} slug="/tools/dev/curl">
      ...
      <div style={card}>
        ...
        <p style={{ marginTop: 12, fontSize: 12, color: 'var(--muted)', lineHeight: 1.7 }}>
          💡 입력·옵션은 자동 저장됩니다(Authorization·Cookie 같은 민감 헤더가 든 입력은 저장하지 않음). <strong>Chrome DevTools → Network → 우클릭 → Copy as cURL</strong> 결과를 그대로 붙여넣어도 됩니다.
        </p>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 540 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>옵션</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>긴 형식</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>의미</th>
              </tr>
            </thead>
            <tbody>
              {[
      ...
      <details style={faqDetails}>
        ...
        <div style={faqAnswer}>
          <strong>네, 지원</strong>합니다. Chrome/Firefox DevTools의 &quot;Copy as cURL&quot;이 만드는 다중 라인 형식을 그대로 붙여넣으면 됩니다.<br />
          예시:
...
{`curl 'https://api.example.com/data' \\
  ...
  --compressed`}
          ...
          • 일반 한 줄<br />
          • Chrome이 특수문자가 든 본문에 쓰는 <code style={codeStyle}>{`$'…'`}</code> 형식<br />
          파서는 줄 이어짐을 공백으로 변환 후 토큰화합니다. 셸 변수 치환(<code style={codeStyle}>$VAR</code>)은 처리하지 않습니다.
        </div>
      </details>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          • <strong>Node.js</strong>: form-data 패키지 + <code style={codeStyle}>fs.createReadStream</code> 코드 생성<br />
          • <strong>Go</strong>: mime/multipart Writer + <code style={codeStyle}>os.Open</code> 코드 생성<br />
          파일 경로와 브라우저 파일 입력 부분은 환경에 맞게 바꿔 쓰세요. <strong>비-multipart 파일</strong> (<code style={codeStyle}>-d @body.txt</code>)는 <strong>미지원</strong>이며 경고 표시.
        </p>
      </details>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          • <strong>Node.js</strong>: form-data 패키지로 본문 생성 (<code style={codeStyle}>form.getHeaders()</code>)<br />
          • <strong>Go</strong>: mime/multipart Writer로 본문 생성 (<code style={codeStyle}>FormDataContentType()</code>)<br />
          <strong>주의</strong>: cURL의 <code style={codeStyle}>-F</code>는 자동으로 <code style={codeStyle}>Content-Type: multipart/form-data; boundary=...</code>를 생성합니다. 사용자가 직접 -H로 Content-Type을 지정하면 boundary 누락으로 깨질 수 있어요. 본 도구는 multipart 본문일 때 입력한 <strong>Content-Type 헤더를 5개 언어 코드 모두에서 제거</strong>합니다.
        </p>
      </details>
    ...
    "variables": {}
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          • 입력은 <strong>localStorage에만 저장</strong> (재방문 편의), 외부 전송 X. Authorization·Cookie·API 키 헤더나 -u 인증이 든 입력은 저장하지 않고 옵션만 저장<br />
          <strong>다만</strong>: 공용 PC·공유 기기에서 토큰을 다뤘다면 사용 후 정리하세요.
