...
const FAQ_LD = [
  ...
  { "q":"Python에선 requests vs httpx 차이?","a":"requests: • 가장 널리 쓰이는 HTTP 클라이언트 (2011~) • 동기 (sync) 만 지원 • 단순, 직관적, 풍부한 문서 httpx: • requests 호환 API + 동기·비동기 모두 지원 • HTTP/2 기본 지원 • FastAPI 등 비동기 프로젝트에 적합 본 도구는 requests 코드를 생성합니다 (가장 일반적). httpx 사용자는 import 부분만 변경하면 거의 호환: . 비동기는 사용." },
  { "q":"Node.js fetch는 언제부터?","a":"Node.js 18 (2022.04)부터 fetch가 전역으로 사용 가능 (node-fetch 패키지 불필요). • Node 18~20: 실험적 (--experimental-fetch 플래그 없이 사용) • Node 21+: 안정 stable • Node 16 이하: npm i node-fetch 필요 본 도구의 fetch 코드는 Node 18+ 가정. 구버전이면: 또는 axios·node http 코드를 사용하세요." },
  { "q":"다중 라인 cURL (\\) 지원하나요?","a":"네, 완벽히 지원합니다. Chrome/Firefox DevTools의 \"Copy as cURL\"이 생성하는 다중 라인 형식 그대로 붙여넣어도 동작합니다. 예시: 지원 형식: • + 개행 (셸 표준) • + CRLF (Windows 친화) • 일반 한 줄 파서는 줄 이어짐을 공백으로 변환 후 토큰화합니다." },
  { "q":"파일 업로드(-F)는 변환되나요?","a":"multipart 구조는 지원, 파일 자체는 placeholder입니다. • -F \"file=@photo.jpg\" → 코드 생성: FormData에 파일 placeholder • fetch / axios: 형식 (브라우저 파일 입력 가정) • Python: 자동 추가 • Node.js / Go: 주석으로 form-data 패키지 또는 mime/multipart 사용 안내 실제 파일 처리 코드는 사용자가 직접 추가해야 합니다 (환경별로 다름). 비-multipart 파일 (-d @body.txt)는 미지원이며 경고 표시." },
  { "q":"Authorization 토큰을 안전하게 처리하려면?","a":"핵심 원칙: 토큰을 코드에 직접 하드코딩 X, 환경 변수로 분리. • JavaScript (브라우저): 절대 클라이언트 코드에 X. 백엔드 프록시 경유 • Node.js: process.env.API_TOKEN + .env 파일 (.gitignore) • Python: + python-dotenv • Go: + .env • CI/CD: GitHub Actions Secrets·GitLab CI Variables • 운영: AWS Secrets Manager·Vault·Doppler 등 시크릿 매니저 본 도구의 마스킹 옵션을 켜면 코드 출력 시 ***로 표시되어 우연한 노출을 방지합니다." },
  ...
  { "q":"GraphQL cURL 변환?","a":"GraphQL은 일반 HTTP POST + JSON body이므로 완벽히 변환됩니다. 예시 cURL: {`curl -X POST 'https://api.github.com/graphql' \\\\ -H 'Authorization: Bearer TOKEN' \\\\ -d '{ \"query\": \"query { viewer }\", \"variables\": }'`} 본 도구의 탭 3 예시에서 GraphQL 카드를 적용해 보세요. query·variables 필드가 포함된 JSON이 5 언어로 변환됩니다. 더 정교한 GraphQL 클라이언트(Apollo·urql·graphql-request)는 별도 라이브러리 사용 권장." },
