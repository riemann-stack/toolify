...
const FAQ_LD = [
  ...
  { "q":"다중 라인 cURL (\\) 지원하나요?","a":"네, 완벽히 지원합니다. Chrome/Firefox DevTools의 \"Copy as cURL\"이 생성하는 다중 라인 형식 그대로 붙여넣어도 동작합니다. 지원 형식은 백슬래시+개행(셸 표준), 백슬래시+CRLF(Windows 친화), 일반 한 줄 세 가지이며, 파서는 줄 이어짐을 공백으로 변환한 뒤 토큰화합니다." },
  { "q":"파일 업로드(-F)는 변환되나요?","a":"multipart 구조는 지원하며 파일 자체는 placeholder로 생성됩니다. -F \"file=@photo.jpg\"는 fetch/axios에서 FormData에 파일을 추가하는 형식(브라우저 파일 입력 가정)으로, Python에서는 open('photo.jpg', 'rb')가 자동 추가되고, Node.js/Go는 form-data 패키지 또는 mime/multipart 사용을 주석으로 안내합니다. 실제 파일 처리 코드는 환경별로 달라 사용자가 직접 추가해야 하며, 비-multipart 파일(-d @body.txt)은 미지원으로 경고가 표시됩니다." },
  ...
  { "q":"multipart/form-data 변환?","a":"-F 옵션이 multipart로 자동 변환됩니다. • fetch / axios: FormData 객체 생성. Content-Type 헤더 직접 설정 금지 (boundary 자동 생성) • Python: files= + data= 분리 • Node.js / Go: form-data 패키지 또는 mime/multipart 사용 안내 주의: cURL의 -F는 자동으로 Content-Type: multipart/form-data; boundary=...를 생성합니다. 사용자가 직접 -H로 Content-Type을 지정하면 boundary 누락으로 깨질 수 있어요. 본 도구는 multipart 시 Content-Type 헤더를 자동 제거합니다." },
  ...
  { "q":"본 도구는 입력 데이터를 서버에 보내나요?","a":"아니요. 모든 처리가 브라우저(클라이언트)에서 수행됩니다. • cURL 파싱: 직접 작성한 토큰화 + 플래그 파서 (~500줄, 외부 라이브러리 0) • 코드 생성: 템플릿 문자열 (서버 호출 0) • Network 탭 확인: 변환 시 어떤 fetch/XHR도 발생하지 않음 • 입력은 localStorage에만 저장 (재방문 편의), 외부 전송 X 다만: 공용 PC·공유 기기에서 Authorization 토큰·API 키를 다룬 경우 사용 후 정리하세요. DevTools → Application → Local Storage에서 youtil_curl_v1 키 삭제 가능. 민감 토큰은 마스킹 옵션으로 코드에서 *** 처리하고, 실제 값은 환경 변수로 옮기는 것을 권장합니다." }
]
