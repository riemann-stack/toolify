...
const FAQ_LD = [
  ...
  { "q":"CORS 오류는 몇 번 코드인가요?","a":"고정된 코드 없음 — 상황에 따라 다릅니다. • preflight OPTIONS 미응답 → 405 Method Not Allowed 또는 응답 자체 없음 • Allow-Origin 헤더 누락 → 200 OK이지만 브라우저가 차단 (네트워크 탭은 200, 콘솔은 CORS 오류) • credentials: include + Allow-Origin: * → 200이지만 브라우저 차단 해결: 본 도구의 [🐛 디버깅] 탭에서 \"CORS preflight 실패\" 시나리오 참조. 핵심: 서버에 ·· 설정." },
  ...
  { "q":"422 Unprocessable Entity는 언제?","a":"400과 다른 점이 핵심: • 400 Bad Request: 구문 오류 (JSON 파싱 실패·콤마 누락) • 422 Unprocessable Entity: 구문은 맞지만 의미 검증 실패 (이메일 형식 X·필수 필드 빈 값·비즈니스 규칙 위반) 프레임워크 자동 사용: • FastAPI: Pydantic validation 실패 시 자동 422 • Spring Boot: @Valid 실패 시 기본 400 (수동 422 권장) • Rails ActiveRecord: validation 실패 시 422 REST 베스트 프랙티스는 422 사용으로 의미 명확화. 응답 본문에 필드별 오류 배열 포함: {`{ \"detail\": [ ]}`}" },
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 480 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>카테고리</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>범위</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>의미</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>대표</th>
              </tr>
            </thead>
            <tbody>
              {[
      ...
      <div style={card}>
        ...
        <div style={{ background: 'var(--bg3)', borderRadius: 10, padding: '14px 16px', marginTop: 12, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 480 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>프레임워크</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>패턴·실패</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11 }}>기본 응답</th>
              </tr>
            </thead>
            <tbody>
              {[
