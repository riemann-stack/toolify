...
const FAQ_LD = [
  ...
  { "q":"YAML 들여쓰기는 탭 vs 공백?","a":"YAML은 탭 문자를 허용하지 않습니다 (공백만 사용). 탭을 쓰면 파싱 오류가 발생해요. • 표준: 2 spaces (DevOps·K8s·Docker), 4 spaces (Java/Spring 일부) • 한 파일 내 통일 필수 — 2/4 혼용은 들여쓰기 오류 에디터 설정: • VS Code: editor.insertSpaces true, editor.tabSize 2 • IntelliJ: Settings → Editor → Code Style → YAML → Tab and Indents 본 도구는 입력 타입 무관(탭/공백) 파싱하지만, 출력은 공백 2/4만 사용 (YAML 표준 준수)." },
  { "q":"K8s 매니페스트의 ---는 무엇인가요?","a":"YAML 멀티 도큐먼트 구분자입니다. 한 파일에 여러 YAML 도큐먼트를 담을 때 사용해요. K8s에서는 Deployment + Service + ConfigMap을 한 파일에 묶을 때 자주 씁니다. 본 도구는 각 도큐먼트를 원소로 담은 JSON 배열로 변환하며(Deployment 객체·Service 객체가 배열에 나란히 들어감), 멀티 도큐먼트 발견 시 자동 안내합니다." },
  ...
  { "q":"앵커(&)와 별칭(*)이 무엇인가요?","a":"YAML의 참조 메커니즘입니다. 같은 데이터를 여러 곳에서 재사용해 중복을 줄여요. 예를 들어 defaults: &defaults 아래에 timeout: 30, retries: 3 같은 공통값을 두고, prod: 항목에서 별칭 병합(*defaults)으로 끌어와 재사용합니다. JSON 변환 시에는 앵커가 펼쳐져 모든 곳에 데이터가 복사됩니다(참조 관계 손실). 역변환 시 자동 앵커 생성은 본 도구에서 끔(noRefs: true)으로 두어 깔끔한 YAML 출력을 보장합니다." },
  ...
  { "q":"변환 결과가 원본보다 길어요","a":"YAML → JSON: 일반적으로 JSON이 약 10~30% 길어집니다. 이유: • 키마다 따옴표 추가 (name이 \"name\"으로) • 중괄호·대괄호 추가 • 콤마 구분자 추가 • 들여쓰기 (compact 모드는 한 줄로 줄임) JSON → YAML: 일반적으로 YAML이 짧아집니다 (괄호·따옴표 제거). 파일 크기가 중요하면 JSON Compact 모드(한 줄)를 사용하세요. 옵션 패널의 \"JSON 출력: Compact\"." },
...
export default function YamlJsonPage() {
  return (
    <ToolPage width={880} slug="/tools/dev/yaml-json">
      <p style={{ fontSize: '12px', color: 'var(--muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '10px' }}>
        개발자
      </p>
      <h1 style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(28px, 5vw, 42px)', fontWeight: 800, letterSpacing: '-1px', marginBottom: '12px' }}>
        <ToolIconBadge catId="dev" />YAML ↔ JSON 변환기
      </h1>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 480 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--cyan-600)', fontSize: 11 }}>YAML</th>
                <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--teal-600)', fontSize: 11 }}>JSON</th>
              </tr>
            </thead>
            <tbody>
              {[
