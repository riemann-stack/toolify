...
/* 러닝 페이스 계산기 ▸ 「레이스 플랜」 탭 (구 /tools/sports/race-plan 흡수)
   ─ PaceClient가 next/dynamic으로 지연 로드 — 기본 탭 번들에 포함되지 않는다.
   ─ 계산 로직은 racePlanUtils.ts(원본 그대로) — 이 파일은 UI만. */
import { useState } from 'react'
import s from './racePlan.module.css'
import {
...
export default function RacePlanTab() {
  ...
  const clockInvalid = startClock.trim() !== '' && startClockMin == null
  ...
  const handleCopy = async () => {
    ...
    lines.push('youtil.kr/tools/sports/pace?tab=plan')
    try {
      ...
      <div className={s.card}>
        <div className={s.twoCol}>
          ...
          <div className={s.field}>
            ...
            {/* 오류 문구는 live region이 아니라 aria-describedby로 연결 — 한 글자 칠 때마다 낭독이 끼어들지 않게 */}
            <input id="rp-clock" className={s.input} type="text" inputMode="text"
              placeholder="예: 08:00" value={startClock}
              aria-invalid={clockInvalid}
              aria-describedby={clockInvalid ? 'rp-clock-err' : undefined}
              onChange={e => setStartClock(e.target.value.replace(/[^0-9:]/g, ''))} />
            {clockInvalid && (
              <p id="rp-clock-err" className={s.fieldError}>시:분 형식(0~23시)으로 입력하세요. 예: 08:00</p>
            )}
          </div>
        </div>
