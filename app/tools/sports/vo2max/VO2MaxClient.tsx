...
export default function VO2MaxClient() {
  ...
  useEffect(() => {
    try {
      ...
      if (typeof j.weightKg === 'number') setWeightKg(j.weightKg)
      if (typeof j.cooperDistM === 'number') setCooperDistM(j.cooperDistM)
      if (typeof j.mile15Min === 'number') setMile15Min(j.mile15Min)
      if (typeof j.mile15Sec === 'number') setMile15Sec(j.mile15Sec)
      if (typeof j.rockportMin === 'number') setRockportMin(j.rockportMin)
      if (typeof j.rockportSec === 'number') setRockportSec(j.rockportSec)
      if (typeof j.rockportHR === 'number') setRockportHR(j.rockportHR)
      if (typeof j.queensHR === 'number') setQueensHR(j.queensHR)
      if (typeof j.waistCm === 'number') setWaistCm(j.waistCm)
      if (typeof j.hrRest === 'number') setHrRest(j.hrRest)
      if (typeof j.paFreq === 'number') setPaFreq(j.paFreq)
      if (typeof j.paIntensity === 'number') setPaIntensity(j.paIntensity)
      if (typeof j.paDuration === 'number') setPaDuration(j.paDuration)
    } catch {}
  }, [])
  useEffect(() => {
    try {
    ...
    <div className={s.wrap}>
      ...
      <div className={s.card}>
        ...
        <div className={s.bioGrid}>
          ...
          <div className={s.field}>
            <label>나이</label>
            <NumInput value={age} onChange={setAge} min={10} max={90} step={1} unit="세" />
          </div>
          <div className={s.field}>
            <label>체중</label>
            <NumInput value={weightKg} onChange={setWeightKg} min={30} max={150} step={0.5} unit="kg" />
          </div>
        </div>
      </div>
      ...
      <div className={s.card}>
        ...
        {methodId === 'cooper' && (
          <div className={s.singleField}>
            ...
            <NumInput value={cooperDistM} onChange={setCooperDistM} min={500} max={5000} step={50} unit="m" />
        ...
        {methodId === 'mile1_5' && (
          <div className={s.singleField}>
            ...
            <div className={s.timeRow}>
              <NumInput value={mile15Min} onChange={setMile15Min} min={5} max={30} step={1} unit="분" />
              <NumInput value={mile15Sec} onChange={setMile15Sec} min={0} max={59} step={1} unit="초" />
            </div>
            <p className={s.fieldHint}>
        ...
        {methodId === 'rockport' && (
          <>
            <div className={s.field}>
              ...
              <div className={s.timeRow}>
                <NumInput value={rockportMin} onChange={setRockportMin} min={8} max={30} step={1} unit="분" />
                <NumInput value={rockportSec} onChange={setRockportSec} min={0} max={59} step={1} unit="초" />
              </div>
            </div>
            <div className={s.field} style={{ marginTop: 10 }}>
              <label>도착 직후 심박수 (15초 동안 측정 × 4)</label>
              <NumInput value={rockportHR} onChange={setRockportHR} min={80} max={200} step={1} unit="bpm" />
            </div>
            <p className={s.fieldHint}>
        ...
        {methodId === 'queens' && (
          <div className={s.singleField}>
            ...
            <NumInput value={queensHR} onChange={setQueensHR} min={100} max={200} step={1} unit="bpm" />
            <p className={s.fieldHint}>
        ...
        {methodId === 'norway' && (
          <>
            <div className={s.norwayGrid}>
              <div className={s.field}>
                <label>허리둘레 (cm)</label>
                <NumInput value={waistCm} onChange={setWaistCm} min={50} max={150} step={1} unit="cm" />
              </div>
              <div className={s.field}>
                <label>안정시 심박수 (아침 기상 직후)</label>
                <NumInput value={hrRest} onChange={setHrRest} min={40} max={120} step={1} unit="bpm" />
              </div>
            </div>
      ...
      {vo2 > 0 && (
        <div className={s.card}>
          ...
          <div className={s.paceGrid}>
            {[
              { id: 'E', name: 'Easy (회복)', desc: '대화 가능. 70~75% HRmax', sec: paces.E, color: '#3EFF9B' },
              { id: 'M', name: '마라톤',       desc: '풀코스 race pace',         sec: paces.M, color: '#3EC8FF' },
              { id: 'T', name: 'Threshold (역치)', desc: '1시간 race pace · 20~40분', sec: paces.T, color: '#FFD93E' },
              { id: 'I', name: 'Interval',     desc: 'VO₂max 자극 · 3~5분 반복',  sec: paces.I, color: '#FF8C3E' },
              { id: 'R', name: 'Repetition',   desc: '스피드 · 200~600m 반복',    sec: paces.R, color: '#FF6B6B' },
            ].map(p => (
