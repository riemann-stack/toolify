  ...
  return (
    <div className={s.chartWrap}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className={s.chartSvg} preserveAspectRatio="xMidYMid meet">
        {/* y축 라벨 (빠른 페이스 ↑·느린 페이스 ↓) */}
        <text x={padL - 6} y={padT + 8} fontSize="11" fill="var(--muted)" textAnchor="end">
          {fmtPace(yMin)}
        </text>
        <text x={padL - 6} y={padT + innerH - 2} fontSize="11" fill="var(--muted)" textAnchor="end">
          {fmtPace(yMax)}
        </text>
        <text x={padL - 6} y={padT + innerH + 14} fontSize="9" fill="var(--muted)" textAnchor="end" opacity="0.6">
          (빠름↑)
        </text>
          ...
          return (
            <g key={i}>
              ...
              <text x={xStarts[i] + widths[i] / 2} y={y - 6}
                fontSize="11" fill="var(--text)" textAnchor="middle" fontWeight={700}>
                {fmtPace(seg.paceSec)}
              </text>
              {/* x축 거리 */}
