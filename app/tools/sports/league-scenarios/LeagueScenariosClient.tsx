...
/* ── 탭 ── 'scenarios'(기본) = 조별·잔여 경기 경우의 수 / 'season' = 시즌 승점 예측(구 /tools/sports/football-points 301 목적지) */
type Tab = 'scenarios' | 'season'
// ?tab= 딥링크 허용 목록
const TABS: readonly Tab[] = ['season', 'scenarios']

// 시즌 승점 탭 — 지연 로드로 기본 탭 번들 유지
const SeasonTab = dynamic(() => import('./SeasonTab'), {
...
export default function LeagueScenariosClient() {
  ...
  // 시즌 탭은 처음 열 때 마운트하고, 이후 다른 탭으로 가도 숨김만 해 입력(승무패·라이벌)을 유지
  const [seasonMounted, setSeasonMounted] = useState(false)
  const selectTab = (t: Tab) => {
    setTab(t)
    if (t === 'season') setSeasonMounted(true)
  }
  ...
  return (
    <div className={s.wrap}>
      ...
      <div className={s.modeTabs} role="tablist" aria-label="계산 모드">
        <button type="button" role="tab" aria-selected={tab === 'scenarios'}
          className={`${s.modeTab} ${tab === 'scenarios' ? s.modeTabActive : ''}`}
          onClick={() => selectTab('scenarios')}>
          순위 경우의 수<small>조별리그·잔여 경기</small>
        </button>
        <button type="button" role="tab" aria-selected={tab === 'season'}
          className={`${s.modeTab} ${tab === 'season' ? s.modeTabActive : ''}`}
          onClick={() => selectTab('season')}>
          시즌 승점<small>최종 승점·라이벌 추격</small>
        </button>
      </div>

      <div className={s.panel} hidden={tab !== 'scenarios'}>
        {/* ── 대회 프리셋 ── */}
            ...
            <table className={s.standTable}>
              ...
              <tbody>
                {teams.map((t, i) => (
                  <tr key={t.id}>
                    <td>
                      <label htmlFor={`ls-name-${t.id}`} className="srOnly">팀 {i + 1} 이름</label>
                      <input
                        id={`ls-name-${t.id}`}
                    ...
                    {(['win', 'draw', 'loss', 'gf', 'ga'] as const).map((k) => (
                      <td key={k}>
                        <label htmlFor={`ls-${k}-${t.id}`} className="srOnly">
      ...
      {/* ── 시즌 승점 (PPG 최종 승점·목표 승점·라이벌 비교) — 한 번 연 뒤엔 숨김만 해서 입력 유지 ── */}
      {seasonMounted && (
