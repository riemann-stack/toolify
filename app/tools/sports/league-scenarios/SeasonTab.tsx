...
/* 축구 순위·승점 경우의 수 계산기 ▸ 「시즌 승점」 탭 (구 /tools/sports/football-points 흡수)
   ─ LeagueScenariosClient가 next/dynamic으로 지연 로드 — 기본 탭(경우의 수) 번들에 포함되지 않는다.
   ─ 시즌 모드(팀당 38경기 등 긴 리그): 현재 승점·PPG로 최종 승점 예측, 목표 승점 승무패 조합, 라이벌 추격.
     조별리그 잔여 경기 전수 계산(경우의 수 탭)과는 알고리즘이 달라 별도 탭으로 둔다. 계산 로직은 원본 그대로. */
...
export default function SeasonTab() {
  ...
  function handleCopy() {
    const lines = [
      ...
      'youtil.kr/tools/sports/league-scenarios?tab=season',
    ]
  ...
  return (
    <div className={styles.wrap}>
      ...
      {/* ── 탭 ── */}
      <div className={styles.tabs} role="tablist" aria-label="시즌 승점 보기">
      ...
      <div className={styles.card}>
        ...
        {stats.played > totalGames && (
          <div role="alert" style={{ marginTop: 12, padding: '10px 12px', background: 'color-mix(in srgb, var(--warning) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--warning) 30%, transparent)', borderRadius: 'var(--radius-s)', fontSize: 12, color: 'var(--text)', lineHeight: 1.6 }}>
