...
/* 인터벌 훈련 계산기 [이지·LSD] 탭 (?tab=easy) — 구 /tools/sports/lsd(LSD·이지런 페이스 계산기)를 흡수.
   이지 페이스 범위·존2 심박·회색지대(정크 마일) 경고·롱런 보급 플래너. 계산 로직은 구 LsdClient와 동일.
   IntervalTrainingClient가 next/dynamic으로 불러 기본 탭 번들에 포함되지 않는다. */
...
export default function EasyTab() {
  ...
  function handleCopy() {
    ...
    const lines = [
      ...
      'youtil.kr/tools/sports/interval-training?tab=easy',
    ].filter(Boolean)
  ...
  return (
    <div className={s.wrap}>
      <p className={s.tabIntro}>
        최근 기록으로 <strong>적정 이지 페이스 범위·존2 심박</strong>을 구하고, &lsquo;너무 빠른 이지런(정크 마일)&rsquo;을 피하세요. 롱런 보급 플래너 포함.
      </p>
