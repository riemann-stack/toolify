...
const FAQ_LD = [
  { "q":"조리개 1/3 stop과 풀스톱 차이는?","a":"풀스톱은 1.4 → 2 → 2.8 → 4 → 5.6 → 8 → 11 → 16처럼 √2 배수로 변하는 전통 단위입니다(1 stop = 광량 2배). 현대 카메라는 정밀 조절을 위해 1/3 stop(예: f/3.2, f/3.5, f/4) 단위로 세분화돼요. 이 도구는 f/1.0~f/22 사이 28개 조리개 값(풀 + 1/3 stop)을 지원하며, 슬라이더 인덱스 1당 1/3 stop 변화입니다." },
  ...
  { "q":"등가 노출(Equivalent Exposure)이란?","a":"같은 EV 값을 가지는 다른 조합들을 의미합니다. 예: f/4·1/250·ISO 200 = f/2.8·1/500·ISO 200 = f/4·1/500·ISO 400. 모두 같은 밝기지만 효과는 달라요 — 두 번째는 조리개를 연 만큼 배경이 더 흐려지고(보케) 셔터가 빨라져 움직임도 더 잘 멈추며, 세 번째는 셔터를 빠르게 한 대신 ISO를 올려 노이즈가 약간 늘어납니다. 이 도구의 등가 노출 5종은 조리개 변화를 우선해 다양한 효과를 보여줍니다." },
    ...
    <ToolPage width={880} slug="/tools/art/exposure">
            ...
            <tbody>
              {[
                ...
                <tr key={i}>
                  ...
                  <td style={{ padding: '6px 0', fontFamily: 'var(--font-sans)', fontWeight: 700, color: 'var(--accent)' }}>{row[1]}</td>
                  <td style={{ padding: '6px 0', color: 'var(--muted)' }}>{row[2]}</td>
                </tr>
              ))}
            </tbody>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          이 도구는 f/1.0~f/22 사이 28개 조리개 값(풀 + 1/3 stop)을 지원하며, 슬라이더 인덱스 1당 1/3 stop 변화입니다.
        </p>
      </details>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          모두 같은 밝기지만 효과는 달라요 — 두 번째는 조리개를 연 만큼 배경이 더 흐려지고(보케) 셔터가 빨라져 움직임도 더 잘 멈추며, 세 번째는 셔터를 빠르게 한 대신 ISO를 올려 노이즈가 약간 늘어납니다.
