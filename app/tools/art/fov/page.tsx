...
const FAQ_LD = [
  ...
  { "q":"스마트폰 카메라의 \"24mm 환산\"이 의미하는 것?","a":"스마트폰 카메라 센서는 풀프레임보다 훨씬 작아 크롭 팩터가 큽니다 — 최신 플래그십 메인 카메라(1/1.3형 전후)는 약 ×3.5, 보급형이나 보조 카메라(1/2~1/2.8형)는 약 ×5~×7입니다. 그래서 사양표에는 실제 초점거리 대신 35mm 환산 mm가 표기됩니다. 예: 아이폰 Pro 계열 메인 카메라는 보통 24mm 환산(실제 초점거리는 약 6mm 안팎). 풀프레임 24mm 광각 렌즈와 같은 화각을 의미해요. 망원 카메라는 보통 70mm 환산(실제 약 9mm), 초광각은 13mm 환산(실제 약 1.5~2.2mm, 기종별)." },
    ...
    <div style={{ maxWidth: '880px', margin: '0 auto', padding: '60px 24px 80px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 460 }}>
            ...
            <tbody>
              ...
              {SENSORS.map((sm) => [
                sm.label,
                `${sm.width} × ${sm.height}`,
                `×${Number.isInteger(sm.cropFactor) ? sm.cropFactor.toFixed(1) : sm.cropFactor}`,
                `${Number((50 * sm.cropFactor).toFixed(1))}mm`,
              ]).map((row, i) => (
                <tr key={i}>
                  <td style={{ padding: '6px 10px', color: 'var(--text)', fontSize: 13 }}>{row[0]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--muted)', fontFamily: 'Inter, "Noto Sans KR", system-ui, sans-serif' }}>{row[1]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--accent)', fontFamily: 'Inter, "Noto Sans KR", system-ui, sans-serif', fontWeight: 700 }}>{row[2]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--text)', fontFamily: 'Inter, "Noto Sans KR", system-ui, sans-serif', fontWeight: 700 }}>{row[3]}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10, marginTop: 14 }}>
          ...
          <div style={{ background: 'var(--bg3)', borderTop: '3px solid var(--cat-edu)', borderRadius: 10, padding: '12px 14px' }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--muted)', margin: 0, lineHeight: 1.7 }}>
              수평 <strong>40°</strong> · 대각 <strong>47°</strong> · 초점거리가 화면 대각선(43.3mm)에 가까운 표준 화각
            </p>
          </div>
      ...
      <div style={card}>
        ...
        <ul style={{ paddingLeft: 18, margin: '12px 0', fontSize: 13, color: 'var(--muted)', lineHeight: 1.95 }}>
          ...
          <li><strong style={{ color: 'var(--text)' }}>스마트폰 메인(1/1.3형) f/1.8</strong> ≈ 풀프레임 f/6.3 심도(보케 적음)</li>
        </ul>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 460 }}>
            ...
            <tbody>
              {[
                ...
                <tr key={i}>
                  <td style={{ padding: '6px 10px', color: 'var(--accent)', fontFamily: 'Inter, "Noto Sans KR", system-ui, sans-serif', fontWeight: 700 }}>{row[0]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--text)', fontFamily: 'Inter, "Noto Sans KR", system-ui, sans-serif' }}>{row[1]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--text)', fontFamily: 'Inter, "Noto Sans KR", system-ui, sans-serif' }}>{row[2]}</td>
                  <td style={{ padding: '6px 10px', color: 'var(--muted)', fontSize: 13 }}>{row[3]}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          스마트폰 카메라 센서는 풀프레임보다 훨씬 작아 크롭 팩터가 큽니다 — 최신 플래그십 메인 카메라(1/1.3형 전후)는 약 ×3.5, 보급형이나 보조 카메라(1/2~1/2.8형)는 약 ×5~×7입니다. 그래서 사양표에는 실제 초점거리 대신 35mm 환산 mm가 표기됩니다. 예: 아이폰 Pro 계열 메인 카메라는 보통 24mm 환산(실제 초점거리는 약 6mm 안팎). 풀프레임 24mm 광각 렌즈와 같은 화각을 의미해요. 망원 카메라는 보통 70mm 환산(실제 약 9mm), 초광각은 13mm 환산(실제 약 1.5~2.2mm, 기종별).
        </p>
      </details>
