...
const FAQ_LD = [
  ...
  { "q":"안료 농도가 다른데 같은 비율이면 같은 색?","a":"아닙니다. 같은 1ml라도 안료 농도(pigment load)에 따라 결과가 크게 달라집니다. • 전문가 물감(신한 SWC·홀베인·다니엘 스미스): 안료 농도 ↑ → 적은 양으로도 진함 • 학생용 물감(펜텔·칼라플러스): 안료 농도 ↓, 비활성 충전제 ↑ → 같은 양이라도 흐림 • 젤 푸드컬러: 액상보다 4~5배 진함 → 절반 이하로 사용 본 도구의 [분량 환산] 탭에 안료 농도 보정 슬라이더(0.5×~2.0×)가 있으니, 진한 안료는 0.5×, 흐린 안료는 1.5×~2× 적용하세요." },
  { "q":"잉크 혼합 시 주의사항은?","a":"만년필 잉크는 같은 브랜드·같은 베이스끼리만 혼합하세요. 다른 조합은 침전·응집·만년필 막힘 위험이 큽니다. • 안전한 조합: 같은 브랜드 dye 잉크끼리 (예: 다이아민 + 다이아민) • 위험한 조합: dye + pigment, 다른 브랜드, 산성(iron gall) + 알칼리성 • 혼합 전 시린지·작은 용기에서 테스트해 24시간 후 침전 확인 • 만년필 분해 청소가 어려운 모델(파일럿 캡리스 등)에는 혼합 잉크 사용 금지 캘리그래피용 펜촉(Nikko·Brause)은 만년필보다 막힘 영향 적어 자유로운 혼합 가능합니다." },
  ...
  { "q":"레진 안료는 g 단위로 정확해야 하는 이유?","a":"레진(에폭시)은 화학 경화 반응을 거치므로 안료 비율이 너무 높으면 경화 불량이 발생합니다. • 안전 한계: 안료가 레진+경화제 전체 무게의 3% 이하를 권장 • 5% 이상: 끈적임·표면 결함·완전 경화 실패 가능 • 10% 이상: 경화 자체가 안 되거나 며칠 지나도 액상 유지 전자저울(0.01g)로 정밀 계량하고, 일반 물감/잉크 대신 레진 전용 안료(마이카 파우더·알코올 잉크·레진 페이스트)를 사용하세요. 일반 수성 물감은 레진의 화학 결합을 방해해 경화 불량 원인이 됩니다. ⚠️ 작업 시 환기·장갑·고글 필수. 미경화 레진은 피부 자극·알레르기 유발." },
  { "q":"ΔE(델타 E)가 무엇인가요?","a":"두 색이 사람 눈에 얼마나 다르게 보이는지 수치화한 값입니다 (CIELAB 색공간 거리). 작을수록 비슷. • ΔE 0~1: 완벽 — 일반인 거의 구별 불가 • ΔE 1~2: 매우 비슷 — 훈련된 눈만 구별 • ΔE 2~5: 비슷 — 한눈에 비슷, 자세히 보면 차이 • ΔE 5~10: 가능 — 명확히 다르지만 같은 계열 • ΔE 10+: 다름 — 완전히 다른 색 본 도구의 컬러 매칭 탭은 ΔE를 최소화하는 혼합 비율을 brute-force로 탐색합니다. 인쇄·도장·자동차 페인트 업계 표준 지표예요." }
]
