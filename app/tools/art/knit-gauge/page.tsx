...
import {
  convertPattern, sizeToCounts, deriveWidthCm, distributeShaping, fitToRepeat, distributeIncDec,
  FIT_PRESETS, getSize, getBodyPart,
} from './knitGaugeUtils'
...
const FAQ_LD = [
  {
    q: '게이지 스와치가 꼭 필요한가요?',
    a: '<strong>네, 거의 모든 의류·핏이 중요한 작품에 필수</strong>입니다. 같은 실·바늘이라도 사람마다 손 텐션이 달라 게이지가 달라집니다. 스와치 없이 패턴 그대로 떴는데 완성된 스웨터가 +5cm 큰/작은 비극이 자주 발생해요.<br />단, <strong>스와치 생략 가능한 경우</strong>: 스카프·머플러·도일리 등 핏이 중요하지 않은 작품, 본 작품을 풀어 다시 뜨는 게 큰 부담 아닐 때. 그래도 모자·장갑·양말·스웨터·가디건은 반드시 스와치 권장.',
  },
  {
    q: '게이지가 패턴과 다르면 어떻게 하나요?',
    a: '<strong>두 가지 방법</strong>이 있습니다.<br /><strong>① 바늘 호수 조정</strong> (가장 일반적) — 내 게이지가 헐거우면 한 호수 작은 바늘, 빽빽하면 한 호수 큰 바늘로 바꿔 다시 스와치. 보통 1~2호수 차이로 조정 가능.<br /><strong>② 코·단 수 변환</strong> — 본 도구의 [패턴 변환] 탭에서 자동 환산. 예: 패턴 100코 + 패턴 게이지 22코/10cm + 내 게이지 20코/10cm → 91코로 시작.<br />변환은 <strong>단순 도안</strong>(평직·메리야스)에 잘 맞습니다. 케이블·레이스처럼 코 수가 무늬 반복 단위에 맞아야 하는 도안은 [패턴 변환] 탭의 <strong>무늬 반복 보정</strong>에 도안의 조건(예: 8코 반복 + 가장자리 2코)을 넣으면 조건을 만족하는 가장 가까운 코 수를 알려 줍니다.',
  },
  {
    q: '코 게이지와 단 게이지 둘 다 맞춰야 하나요?',
    a: '<strong>코 게이지가 우선</strong>입니다. 코 게이지는 가로 폭(둘레)을 결정하므로 옷의 핏을 좌우해요.<br /><strong>단 게이지</strong>는 길이를 결정하지만, 길이는 <strong>&quot;몇 단&quot;이 아닌 &quot;cm로&quot;</strong> 측정해 마감하면 됩니다 (예: &quot;55cm가 될 때까지 떠 줍니다&quot;).<br />실제 패턴 다수도 길이는 cm로 표기. 패턴 대비 코 게이지 차이(%)와 단 게이지 차이(%)가 15%p 이상 벌어지면 본 도구가 자동으로 경고를 띄워 줍니다 (탭 2).',
  },
  {
    q: '스웨터 시작 코는 가슴둘레 그대로인가요, 절반인가요?',
    a: '<strong>작업 방식에 따라 다릅니다.</strong> 평면(앞·뒤판을 따로 떠 옆선 봉합)이면 한 장의 폭이 <strong>완성 가슴둘레의 절반</strong>이고, 원형(몸통을 한 번에)이면 <strong>완성 가슴둘레 전체</strong>에 게이지를 곱합니다. 같은 옷이라도 원형 시작 코 수가 평면 한 장의 약 2배입니다.<br />발행 패턴으로 확인해 보면 — 평면 패턴(Knitty &#39;Split Decision&#39;)은 9개 사이즈 전부 &#39;완성둘레÷2×게이지 + 2코&#39;로 코를 잡고, 그 +2코가 패턴이 명시한 셀비지(가장자리 각 1코)와 정확히 일치합니다. 원형 패턴(Knitty &#39;Wheel of Life&#39;)은 완성 가슴 34인치 × 5코/인치 = 170코로 코잡기 수와 정확히 같습니다.<br />봉합에는 각 조각 1코씩(이음매당 2코, 옆선 2군데면 4코)이 접혀 들어가므로 평면 두 장 합계는 원형 등가 코수보다 그만큼 많습니다. 다만 셀비지 폭은 실 굵기·기법에 따라 달라 <strong>항상 정확히 4코는 아닙니다.</strong> 본 도구는 [사이즈별] 탭에서 평면/원형을 고르게 하고, 평면일 때 앞뒤 합계도 함께 보여 줍니다.',
  },
  {
    q: '소매 늘림은 몇 단마다 해야 하나요?',
    a: '필요한 정보는 <strong>성형 구간의 총 단 수</strong>, <strong>총 늘릴 코 수</strong>, <strong>한 성형단당 늘리는 코 수</strong>입니다. 소매는 성형단에서 좌우 각 1코씩 총 <strong>2코</strong>를 늘리는 것이 통상이라 성형단 횟수 = 늘릴 총 코 수 ÷ 2입니다. 래글런은 라인 4개에서 각 2코씩 1회 <strong>8코</strong>이며 보통 2단(원형이면 2라운드)마다 반복합니다.<br />간격은 총 단 수 ÷ 성형 횟수로 구하되 나눠떨어지지 않으므로 두 종류의 간격으로 쪼갭니다(통용 명칭 <strong>매직 포뮬러</strong>). 평면 뜨기는 겉면(RS)에서만 성형하는 것이 대부분이라 간격을 <strong>짝수 단으로 내림</strong>하고 나머지도 2단 단위로 얹습니다 — 다만 원저도 이를 엄밀한 규칙이 아니라 &#39;대부분이 선호하는 관행&#39;이라고 밝힙니다. 원형은 겉·안면 교대가 없어 홀수 간격도 그대로 씁니다.<br />검산은 <strong>(간격 × 횟수)의 합이 총 단 수와 같은지</strong> 보면 됩니다. 본 도구 [늘림·실 양] 탭의 세로 모드가 이 계산을 해 줍니다.',
  },
  {
    q: '모자 시작 코는 머리 둘레 그대로?',
    a: '아닙니다. <strong>머리 둘레보다 작게</strong> 떠야 늘어났을 때 잘 맞습니다 (negative ease). 다만 <strong>&#39;얼마나 작게&#39;는 발행처마다 다릅니다.</strong><br />• <strong>Woolly Wormhead</strong>(모자 전문 디자이너, Knitty가 사이징 기준으로 링크): 완성 둘레를 5~7.5cm(2~3인치), 대략 12% 작게<br />• <strong>KnitPicks</strong>: 약 5cm(2인치)<br />• <strong>Interweave</strong>: 딱 붙는 비니는 0~2.5cm(0~1인치)<br />본 도구 기본값 -10%(머리 56cm → 약 50cm)는 이 범위 안이며, [사이즈별] 탭 슬라이더로 조절할 수 있습니다. 네거티브 이즈는 <strong>둘레에만</strong> 적용하는 값이라 모자 높이(22cm 안팎)에는 곱하지 않습니다.',
  },
  {
    q: '양말 발 둘레 코 수는 어떻게?',
    a: '<strong>둘레는 약 10% 작게</strong>가 기준입니다. Knitty의 디자이너 사이징 가이드라인 원문도 &quot;A sock is best worn with about 10% negative ease&quot;입니다.<br />다만 <strong>길이는 다릅니다</strong> — Kate Atherley(『Custom Socks』 저자·Knitty 테크에디터)는 둘레는 약 10%(성인 약 2.5cm) 작게, <strong>길이는 약 1cm만</strong> 짧게 뜨라고 씁니다. 본 도구도 양말 길이에는 -10%가 아니라 -1cm를 적용합니다.<br />• 무늬별 차등(Kate Atherley): 배색(스트랜디드)은 이즈 0, 케이블은 약 5%, 레이스는 15% 이상도<br />• Knitty &#39;Socks 101&#39; 예시: 발목 둘레 8인치(약 20cm)에서 성인은 1인치(아동은 0.5인치)를 빼고 게이지를 곱해 52코로 시작<br />양말은 보통 4-DPN(double-pointed needles) 또는 매직 루프로 원형 작업합니다.',
  },
  {
    q: '한국·미국·일본 단위 차이는?',
    a: '<strong>코 게이지 측정 단위</strong>:<br />• 한국·유럽: <strong>10×10cm</strong><br />• 미국: <strong>4×4 in</strong> (= 10.16cm) — CYC 공식 표는 표 제목에 &quot;to 4 inches / 10 cm&quot;로 두 단위를 함께 적어 같은 칸으로 취급합니다<br />• 일본: <strong>10×10cm</strong> 표준<br />본 도구는 10×10cm·4×4인치·1×1cm 입력을 토글로 지원합니다. 또한 <strong>바늘 호수</strong>도 mm·US·UK 동시 표기 (탭 4).<br />• 미국식 6호 = 4.0mm (CYC 공식 환산표)<br />• <strong>일본 호수는 별도 체계</strong>입니다 — 클로버(Clover) 공식 규정상 0호가 축 2.1mm이고 15호(6.6mm)까지 0.3mm 간격이라 일본 5호 = 3.6mm이며, 15호보다 굵으면 호수 없이 mm(점보침)로만 표기합니다. 서양 사다리와 정확히 겹치는 지점은 3.0mm(3호)·4.5mm(8호)·6.0mm(13호)뿐이라 나머지는 근사입니다.<br />참고로 CYC 표에는 UK 호수 열이 없어, 도구 표의 UK 값은 제조사 환산표를 근거로 합니다. 본 도구는 mm 단위 우선.',
  },
  {
    q: '늘림(M1L)과 (kfb) 차이는?',
    a: '둘 다 1코 늘림이지만 모양이 달라요.<br />• <strong>M1L (Make 1 Left)</strong>: 두 코 사이 가로실을 들어 올려 새 코 만듦. <strong>거의 보이지 않음</strong>. 매끈한 라인 원할 때.<br />• <strong>kfb (Knit Front and Back)</strong>: 한 코의 앞과 뒤에 모두 떠서 1코 더 만듦. <strong>작은 매듭이 보임</strong>. 입문자에게 쉬움.<br />소매 늘림처럼 매끈해야 하는 곳은 M1L/M1R, 매듭이 무늬가 되는 디자인은 kfb. 본 도구의 [늘림·실 양] 탭 약어 용어집에 늘림·줄임 약어 9개(와 겉뜨기 k)의 설명이 있습니다.<br /><strong>한 단에 몰아서 처리할 수 있는 양에는 한계</strong>가 있습니다 — 늘림은 코마다 1코씩 늘려도 최대 <strong>현재 코 수</strong>까지, 줄임은 k2tog 하나가 2코를 소비하므로 최대 <strong>현재 코 수의 절반</strong>까지입니다. 이를 넘으면 도구가 억지 분배 라벨 대신 몇 단에 나눠 진행하라는 안내를 내보냅니다.',
  },
  {
    q: '실 양 계산은 정확한가요?',
    a: '본 도구의 g 추정치는 <strong>통용 관행에 기반한 근사이며 공식 표준이 아닙니다.</strong> CYC 표준집에는 작품별 실 소요량 표 자체가 없고(수록 차트 목록에 해당 항목 부재), 업계 표기는 무게가 아니라 <strong>길이(야드·미터)</strong>입니다. Lion Brand 공식 안내 원문도 &quot;The best and most accurate measure for calculating amounts is yards&quot;이며, 같은 worsted라도 8온스당 최대 100야드 차이가 날 수 있다고 밝힙니다.<br />• 참고 Lion Brand 공식 야드 표: 모자 CYC 4 → <strong>200~225yd</strong> / 양말 CYC 1 → <strong>525~825yd</strong> / 성인 스웨터 CYC 3 → <strong>1,500~2,250yd</strong>, CYC 4 → <strong>1,125~1,625yd</strong> / 아프간 CYC 4 → <strong>2,250~3,125yd</strong><br />• 정확히 계산하려면 실 라벨의 <strong>&quot;100g = ○m&quot;</strong>으로 필요한 길이를 타래 수로 환산하세요.<br />• 그 밖의 변수: 케이블·아란 패턴은 더, 레이스는 덜 드는 편이고, 컬러워크·페어아일은 두 가닥 동시 + 부동사(浮動絲)로 더 필요합니다. 여유분을 넉넉히 두고 같은 다이로트(dye lot)로 한 번에 구입해야 색이 맞습니다.',
  },
  {
    q: '바늘 호수가 게이지에 영향?',
    a: '<strong>네, 가장 큰 영향</strong>입니다. 같은 실이라도 바늘이 굵으면 코가 커져 게이지가 헐거워지고, 바늘이 가늘면 빽빽해집니다.<br />• 한 호수 차이로도 <strong>게이지가 눈에 띄게</strong> 달라짐(실·손 텐션에 따라 폭은 제각각)<br />• 게이지가 패턴과 안 맞으면 <strong>실 바꾸지 말고 바늘 호수 먼저 조정</strong><br />• 본 도구의 [늘림·실 양] 탭 마지막 표에 바늘 호수 ↔ 게이지 룩업 표가 있어 권장 호수를 바로 확인 가능합니다.<br />또한 <strong>바늘 재질</strong>(스틸·대나무·플라스틱)도 미세하게 영향. 매끄러운 스틸이 더 빨리 뜨이지만 텐션이 헐거워질 수 있어요.',
  },
  {
    q: '코바늘과 대바늘 게이지 차이?',
    a: '<strong>같은 실이라도 코바늘이 더 두껍고 빡빡한 편물</strong>이 됩니다 (코 단위가 더 큼).<br />• 같은 DK 실: 대바늘 22코/10cm, 코바늘 12~17코/10cm(CYC 3 Light 짧은뜨기 규정 범위)<br />• 코바늘은 <strong>한 코의 높이가 더 큼</strong> → 단 게이지가 코 게이지와 더 비슷<br />• 코바늘 패턴은 <strong>&quot;짧은뜨기·긴뜨기·한길긴뜨기&quot;</strong> 등 뜨기 종류별로 게이지가 달라, 패턴에 명시된 뜨기로 측정<br />본 도구의 게이지 입력은 코바늘·대바늘 모두 사용 가능하지만, 코바늘 사용 시에는 패턴이 명시한 뜨기 종류와 동일하게 스와치를 떠 측정하세요.',
  },
]

/* ── 가이드 '계산 공식' 표 — 도구와 같은 함수로 빌드 시 계산(손으로 적은 숫자 아님) ── */
const EX_M = getSize('female_m')
const EX_CLASSIC = FIT_PRESETS.find((f) => f.id === 'classic')
const EX_BUST = EX_M?.bust ?? 92
const EX_EASE = EX_CLASSIC?.applyCm ?? 7.5
const EX_FINISHED = EX_BUST + EX_EASE
/* 화면 입력칸과 같은 0.1cm 반올림(deriveWidthCm)을 거쳐야 도구 결과와 코 수가 일치한다 */
const EX_FLAT_W = deriveWidthCm(EX_BUST, EX_EASE, 'flat')
const EX_ROUND_W = deriveWidthCm(EX_BUST, EX_EASE, 'round')
const EX_FLAT = sizeToCounts(EX_FLAT_W, 60, 22, 30)
const EX_ROUND = sizeToCounts(EX_ROUND_W, 60, 22, 30)
const EX_HAT_PART = getBodyPart('hat')
const EX_HAT = sizeToCounts(EX_HAT_PART.defaultW, EX_HAT_PART.defaultH, 22, 30, EX_HAT_PART.ease ?? 0)
const EX_CONV = convertPattern({ patternStsGauge: 22, patternRowsGauge: 30, patternSts: 100, patternRows: 120, myStsGauge: 20, myRowsGauge: 28 })
const EX_ROW = distributeIncDec(90, 100)
const EX_SLEEVE = distributeShaping(100, 48, 72, 2, true)
const EX_REPEAT = fitToRepeat(EX_CONV.newSts, 8, 2)
const fmt1 = (n: number) => (Math.round(n * 10) / 10).toLocaleString('ko-KR')
const FORMULA_ROWS: { name: string; formula: string; input: string; result: string }[] = [
  {
    name: '패턴 변환',
    formula: '새 코 수 = 패턴 코 ÷ 패턴 게이지 × 내 게이지 (단도 같은 식, 반올림)',
    input: '패턴 100코·120단, 패턴 22코/30단 → 내 20코/28단',
    result: `${EX_CONV.newSts}코 · ${EX_CONV.newRows}단 (코 게이지 ${fmt1(EX_CONV.stsDeltaPct)}%)`,
  },
  {
    name: '스웨터 몸통',
    formula: '완성 둘레 = 가슴둘레 + 여유분 → 평면은 ÷2, 원형은 그대로 × 게이지 ÷ 10',
    input: `가슴 ${EX_BUST}cm + 클래식 ${EX_EASE}cm = ${fmt1(EX_FINISHED)}cm, 22코/10cm`,
    result: `평면 한 장 ${fmt1(EX_FLAT_W)}cm → ${EX_FLAT.sts}코 · 원형 ${fmt1(EX_ROUND_W)}cm → ${EX_ROUND.sts}코`,
  },
  {
    name: '모자 둘레',
    formula: '코 수 = 머리 둘레 × (1 + 이즈) × 게이지 ÷ 10 (높이엔 이즈 미적용)',
    input: `머리 ${EX_HAT_PART.defaultW}cm, 이즈 ${Math.round((EX_HAT_PART.ease ?? 0) * 100)}%, 높이 ${EX_HAT_PART.defaultH}cm, 22코/30단`,
    result: `${EX_HAT.sts}코(짝수 권장 ${EX_HAT.evenSts}코) · ${EX_HAT.rows}단`,
  },
  {
    name: '한 단 균등 분배',
    formula: '간격 = 현재 코 ÷ 늘릴 코 수 (나머지는 한 코 긴 간격으로)',
    input: '현재 90코 → 목표 100코',
    result: EX_ROW.label,
  },
  {
    name: '소매 세로 성형',
    formula: '성형 횟수 = 늘릴 코 ÷ 성형단당 코, 간격 = 총 단 ÷ 횟수 (겉면만이면 짝수 단)',
    input: '100단 동안 48코 → 72코, 성형단당 2코, 겉면에서만',
    result: EX_SLEEVE.label,
  },
  {
    name: '무늬 반복 보정',
    formula: '코 수 = 반복 단위 × n + 가장자리 코 중 가장 가까운 값',
    input: `환산 ${EX_CONV.newSts}코, 8코 반복 + 2코`,
    result: EX_REPEAT.label,
  },
]
...
export default function KnitGaugePage() {
  return (
    <ToolPage width={880} slug="/tools/art/knit-gauge">
      ...
      {/* 면책 박스 */}
      <Callout tone="warn">
        본 도구는 표준 게이지·사이즈 기반 <strong>어림 계산</strong>입니다. 실 종류·블로킹 후 변화·개인 손 텐션에 따라 결과가 달라집니다.
        실제 작업은 반드시 <strong>게이지 스와치를 떠서 측정한 실제 게이지</strong>로 작업하세요.
        분야별 안전 안내는 <Link href="/disclaimer#art">면책조항</Link> 참고.
      </Callout>
      ...
      <div style={card}>
        ...
        <p className="g-note">
          모든 입력값(게이지·치수·실 종류 등)은 이 브라우저에 자동 저장되어 새로고침해도 유지됩니다. 뜨개질 중 휴대폰으로 열어 두고 쓰기 좋습니다.
        </p>
      </div>

      {/* 1b. 계산 공식 — utils로 빌드 시 계산 */}
      <h2 className="g-h2">계산 공식 — 도구가 실제로 하는 계산과 예시</h2>
      <p className="g-p">
        모든 계산의 출발점은 <strong>1cm당 코 수 = 10cm 게이지 ÷ 10</strong>입니다. 여기에 원하는 폭(cm)을 곱하면 코 수, 원하는 길이를 곱하면 단 수가 나오고,
        패턴 변환은 &lsquo;패턴 게이지로 떴을 때의 cm&rsquo;를 먼저 구한 뒤 내 게이지로 다시 코 수를 셉니다. 아래 표의 결과 칸은 이 페이지를 만들 때
        계산기와 <strong>같은 함수</strong>로 계산한 값이라, 같은 값을 입력하면 도구 화면에도 똑같이 나옵니다.
      </p>
      <div className="tableScroll">
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 640 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 12 }}>계산</th>
              <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 12 }}>공식</th>
              <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 12 }}>예시 입력</th>
              <th scope="col" style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 12 }}>도구 결과</th>
            </tr>
          </thead>
          <tbody>
            {FORMULA_ROWS.map((r) => (
              <tr key={r.name} style={{ borderBottom: '1px solid var(--border)' }}>
                <td style={{ padding: '8px 10px', color: 'var(--text)', fontWeight: 700, whiteSpace: 'nowrap' }}>{r.name}</td>
                <td style={{ padding: '8px 10px', color: 'var(--muted)' }}>{r.formula}</td>
                <td style={{ padding: '8px 10px', color: 'var(--text)' }}>{r.input}</td>
                <td style={{ padding: '8px 10px', color: 'var(--accent-ink)', fontWeight: 700 }}>{r.result}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p className="g-p" style={{ marginTop: 16 }}>
        결과를 읽을 때 자주 틀리는 부분은 세 가지입니다. 첫째, 스웨터는 <strong>평면 한 장 {EX_FLAT.sts}코와 원형 {EX_ROUND.sts}코</strong>처럼 작업 방식에 따라 시작 코가 두 배 차이 나므로
        도안이 앞판·뒤판을 따로 뜨는지 먼저 확인해야 합니다. 둘째, 패턴 변환에서 코 게이지 차이가 ±8%를 넘으면 도구가 바늘 호수 조정을 먼저 권하는데, 코 수를 바꾸는 것보다
        바늘을 바꿔 도안 게이지에 맞추는 편이 성형 위치·무늬를 그대로 쓸 수 있어 안전합니다. 셋째, 환산 코 수({EX_CONV.newSts}코)는 무늬 반복 단위와 맞지 않는 경우가 많아
        케이블·고무단 도안이라면 반드시 반복 보정으로 {EX_REPEAT.down}코나 {EX_REPEAT.up}코처럼 조건에 맞는 값으로 옮겨야 무늬가 끊기지 않습니다.
      </p>

      {/* 2. 게이지 스와치 만드는 법 */}
      <h2 className="g-h2">게이지 스와치 — 왜 필요한가?</h2>
      <div style={card}>
        <p className="g-p">
        ...
        <Callout tone="warn" title="흔한 실수 5가지">
          <ol>
            <li>스와치를 너무 작게(5×5cm) 떠서 측정 — 가장자리 영향으로 부정확</li>
            <li>블로킹 생략 — 실 종류에 따라 블로킹 전후 게이지가 달라짐</li>
            <li>편물을 잡아당기거나 뭉친 채 측정 — 자연스러운 상태에서 핀으로 고정</li>
            <li>실을 바꿔도 같은 바늘 사용 — 실마다 권장 바늘이 다름</li>
            <li>스와치를 본판과 다른 무늬로 뜸 — 본판에 쓰는 무늬 그대로 떠서 측정</li>
          </ol>
        </Callout>
      </div>

      {/* 3. CYC 실 굵기 표 */}
      <h2 className="g-h2">CYC 실 굵기 표 (0~7)</h2>
      <div style={card}>
        <p className="g-p">
        ...
        <Callout tone="note" title="Aran은 CYC의 독립 등급이 아닙니다">
          <p>
            CYC 표에서 Aran은 <strong>4번(Medium)의 실 종류</strong>로 한 번 등장할 뿐입니다(원문 &quot;Worsted, Afghan, Aran&quot;).
          ...
          <p>CYC는 &quot;Size 8&quot; 신설을 예고해 두었지만 2026년 8월 현재 공개 표는 0~7까지이고 8번의 게이지·바늘 수치는 아직 공표되지 않았습니다.</p>
        </Callout>
      ...
      {/* 4. 한국 표준 사이즈 표 */}
      <h2 className="g-h2">뜨개 패턴용 참고 치수 (한국 기준)</h2>
      <div style={card}>
        <p className="g-p">
        ...
        <Callout tone="note" title="KS 의류 치수 표준은 대상별로 번호가 다릅니다">
          <p>
            성인 남성복 <strong>KS K 0050</strong>(만 18세 이상) · 성인 여성복 <strong>KS K 0051</strong>(만 18~59세, 파운데이션 제외) ·
          ...
          <p>
            또 KS 호칭 숫자는 완성된 옷의 치수가 아니라 <strong>착용자의 인체 치수 조합</strong>입니다.
      ...
      {/* 4b. 완성 여유분 (ease) */}
      <h2 className="g-h2">완성 여유분(ease) — 실제 가슴둘레 그대로 뜨면 몸에 붙습니다</h2>
      <div style={card}>
        <p className="g-p">
      ...
      {/* 6. 실 양 — g vs 야드 */}
      <h2 className="g-h2">실 양은 g보다 야드·미터가 정확합니다</h2>
      <div style={card}>
        <p className="g-p">
