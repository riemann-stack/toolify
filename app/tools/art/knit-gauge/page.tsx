...
const FAQ_LD = [
  { "q":"게이지 스와치가 꼭 필요한가요?","a":"네, 거의 모든 의류·핏이 중요한 작품에 필수입니다. 같은 실·바늘이라도 사람마다 손 텐션이 달라 게이지가 5~15% 차이 납니다. 스와치 없이 패턴 그대로 떴는데 완성된 스웨터가 +5cm 큰/작은 비극이 자주 발생해요. 단, 스와치 생략 가능한 경우: 스카프·머플러·도일리 등 핏이 중요하지 않은 작품, 본 작품을 풀어 다시 뜨는 게 큰 부담 아닐 때. 그래도 모자·장갑·양말·스웨터·가디건은 반드시 스와치 권장." },
  ...
  { "q":"모자 시작 코는 머리 둘레 그대로?","a":"아닙니다. 머리 둘레보다 10% 작게 떠야 늘어났을 때 잘 맞습니다 (negative ease). • 머리 둘레 56cm → 시작 둘레 약 50cm로 작업 → 게이지 적용해 코 수 계산 • 본 도구의 [📏 사이즈별] 탭에서 모자 선택 시 신축 보정 -10%가 자동 적용 탄성이 큰 1×1·2×2 코 텐션 무늬(K1P1·K2P2 ribbing)는 -15%까지도 가능. 늘어남이 적은 가터·시드 스티치는 -5%만." },
  { "q":"양말 발 둘레 코 수는 어떻게?","a":"모자와 동일하게 발 둘레보다 10% 작게가 표준입니다. • 발 둘레 22cm (성인 여성 평균) → 시작 둘레 약 20cm로 작업 • Fingering(양말실) 게이지 30코/10cm → 60코 시작이 일반적 • 양말은 보통 4-DPN(double-pointed needles) 또는 매직 루프 원형 작업 발등(instep)과 발바닥(sole)은 보통 같은 코 수, 발꿈치(heel) 부분만 절반의 코로 작업하는 게 표준 구조입니다." },
  { "q":"한국·미국·일본 단위 차이는?","a":"코 게이지 측정 단위: • 한국·유럽: 10×10cm (4×4 in와 거의 동일) • 미국: 4×4 in (≒ 10.16×10.16cm) • 일본: 10×10cm 표준 본 도구는 셋 다 토글로 지원합니다. 또한 바늘 호수도 mm·US·UK 동시 표기 (탭 4). • 미국식 6호 = 4.0mm = 영국식 8호 • 일본식 호수는 mm 표기와 약간 달라 (일본 5호 = 3.6mm). 본 도구는 mm 단위 우선." },
  { "q":"늘림(M1L)과 (kfb) 차이는?","a":"둘 다 1코 늘림이지만 모양이 달라요. • M1L (Make 1 Left): 두 코 사이 가로실을 들어 올려 새 코 만듦. 거의 보이지 않음. 매끈한 라인 원할 때. • kfb (Knit Front and Back): 한 코의 앞과 뒤에 모두 떠서 1코 더 만듦. 작은 매듭이 보임. 입문자에게 쉬움. 소매 늘림처럼 매끈해야 하는 곳은 M1L/M1R, 매듭이 무늬가 되는 디자인은 kfb. 본 도구의 [📊 늘림] 탭 약어 용어집에 8개 기법 설명 있습니다." },
  { "q":"실 양 계산은 정확한가요?","a":"본 도구의 실 양은 표준 평직·메리야스 작업 기준 평균값입니다. 실제로는 다음 변수에 따라 ±20% 차이가 있어요. • 케이블·아란 패턴: 코가 많이 꼬여 평직보다 +20~30% 더 필요 • 컬러워크·페어아일: 두 가닥 동시 + 부동사(浮動絲)로 +30% 추가 • 레이스·튜닉: 평직보다 -10% 적게 • 여유분: 항상 10~15% 더 구입 권장 (수정·실 부족 대비). 같은 다이로트(dye lot)로 한 번에 구입해야 색 일치." },
  { "q":"바늘 호수가 게이지에 영향?","a":"네, 가장 큰 영향입니다. 같은 실이라도 바늘이 굵으면 코가 커져 게이지가 헐거워지고, 바늘이 가늘면 빽빽해집니다. • 한 호수(0.5mm) 차이로 보통 1~2코/10cm 변화 • 게이지가 패턴과 안 맞으면 실 바꾸지 말고 바늘 호수 먼저 조정 • 본 도구의 [📊 늘림·실 양] 탭 마지막 표에 바늘 호수 ↔ 게이지 룩업 표가 있어 권장 호수를 바로 확인 가능합니다. 또한 바늘 재질(스틸·대나무·플라스틱)도 미세하게 영향. 매끄러운 스틸이 더 빨리 뜨이지만 텐션이 헐거워질 수 있어요." },
  { "q":"코바늘과 대바늘 게이지 차이?","a":"같은 실이라도 코바늘이 더 두껍고 빡빡한 편물이 됩니다 (코 단위가 더 큼). • 같은 DK 실: 대바늘 22코/10cm, 코바늘 16~18코/10cm (단코 기준) • 코바늘은 한 코의 높이가 더 큼 → 단 게이지가 코 게이지와 더 비슷 • 코바늘 패턴은 \"단코·반쪽단코·긴뜨기·긴긴뜨기\" 등 종류별로 게이지가 달라, 패턴 명시 단 종류로 측정 본 도구의 게이지 입력은 코바늘·대바늘 모두 사용 가능하지만, 코바늘 사용 시에는 패턴이 명시한 단 종류와 동일하게 스와치를 떠 측정하세요." }
]
    ...
    <div style={{ maxWidth: '880px', margin: '0 auto', padding: '60px 24px 80px' }}>
      ...
      <details style={faqDetails}>
        ...
        <p style={faqAnswer}>
          ...
          • 한국·유럽: <strong>10×10cm (4×4 in와 거의 동일)</strong><br />
          • 미국: <strong>4×4 in</strong> (≒ 10.16×10.16cm)<br />
