/* ─────────────────────────────────────────────────────────
   더미 텍스트·데이터 생성 엔진 (순수 함수 — 노드 검산 가능)
   ⚠️ 클라이언트에서 분리한 이유: 안전 대역(RFC 2606 이메일 · 미부여 010 대역)과
      포맷터 이스케이프는 회귀가 조용히 나는 부분이라 별도 검산이 필요하다.
   ───────────────────────────────────────────────────────── */

/* ───────── 타입 ───────── */
export type Tab = 'paragraph' | 'ui' | 'json' | 'card' | 'ux' | 'length'
export type Tone = 'default' | 'friendly' | 'professional' | 'commerce' | 'saas' | 'finance' | 'health' | 'education' | 'game'
export type Lang = 'ko' | 'en'
export type LengthKey = 'veryShort' | 'short' | 'medium' | 'long' | 'veryLong'
...
export type FormatType = 'json' | 'jsonl' | 'csv' | 'yaml' | 'markdown' | 'html' | 'jsx' | 'ts'
export type Scenario = 'login' | 'signup' | 'payment' | 'delete' | 'empty'

/* ───────── 톤 옵션 ───────── */
export const TONE_OPTIONS: { key: Tone; label: string }[] = [
...
/* ───────── 길이 프리셋 ───────── */
export const LENGTH_PRESETS: { key: LengthKey; label: string; sentences: number; targetChars: number }[] = [
...
/* ───────── 한글 더미 풀 ───────── */
export const TITLE_POOLS: Record<Tone, string[]> = {
...
export const SUBTITLE_POOLS: Record<Tone, string[]> = {
...
export const BUTTON_POOLS: Record<Tone, string[]> = {
...
export const CARD_TITLE_POOLS: Record<Tone, string[]> = {
...
export const CARD_DESC_POOLS: Record<Tone, string[]> = {
...
export const PRODUCT_NAME_POOLS: string[] = [
...
export const PRODUCT_DESC_POOLS: string[] = [
...
export const REVIEW_POOLS: string[] = [
...
export const COMMENT_POOLS: string[] = [
  '정말 유용한 정보 감사합니다!',
...
export const NAME_POOLS: string[] = [
  '김민수', '이지은', '박서준', '최유진', '정현우',
  '강하늘', '윤소희', '조현준', '한지민', '서민호',
  '오세영', '신예린', '임도윤', '배지원', '권태현',
]

export const NICKNAME_POOLS: string[] = [
...
export const NOTIFICATION_POOLS: Record<Tone, string[]> = {
...
export const ERROR_POOLS: Record<Tone, string[]> = {
...
export const EMPTY_POOLS: Record<Tone, string[]> = {
...
export const ONBOARDING_POOLS: Record<Tone, string[]> = {
...
export const PRICING_POOLS: Record<Tone, string[]> = {
...
export const FAQ_Q_POOLS: string[] = [
...
export const FAQ_A_POOLS: string[] = [
...
/** 균등 셔플 (Fisher-Yates).
    ⚠️ `sort(() => Math.random() - 0.5)`는 비교 함수가 비일관이라 결과가 편향된다 —
       5개 배열 6만 회 실측에서 첫 자리 분포가 A 32% / B 12%로 갈렸다(균등이면 각 20%). */
...
/** 로컬(KST) 기준 날짜. ⚠️ toISOString()은 UTC라 KST 00~09시에 하루 전 날짜가 나온다. */
export function localDateStr(d: Date) {
  return `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`
}
export function randomDateISO() {
  const d = new Date()
  d.setDate(d.getDate() - randInt(0, 365))
  return localDateStr(d)
}
export function localDateTimeStr(d: Date) {
  return `${localDateStr(d)} ${pad2(d.getHours())}:${pad2(d.getMinutes())}`
}
export function randomDateTimeISO() {
  const d = new Date()
  d.setDate(d.getDate() - randInt(0, 365))
...
/* 더미 이메일.
   ⚠️ 예전에는 gmail.com·naver.com·kakao.com·youtil.kr을 섞어 썼다. 이 도구가 스스로
      "생성한 JSON을 json-server·MSW에 그대로 붙여 넣으라"고 안내하므로, 시드로 들어간
      주소로 실제 메일이 발송될 수 있는 도메인을 쓸 이유가 없다.
      RFC 2606 §3이 문서·예시용으로 예약한 example.com/net/org만 쓴다 —
      이 도메인들은 RFC 7505 널 MX(`0 .`)를 공표해 배달 자체가 불가능하다. */
export const DUMMY_EMAIL_DOMAINS = ['example.com', 'example.net', 'example.org']
export function randomEmail(name: string) {
  /* 한글 이름을 그대로 로컬파트에 쓰면 대부분의 메일 시스템에서 무효한 주소가 된다 —
     라틴 문자 핸들로 옮겨 형식이 유효한 더미를 만든다. */
  const handle = /^[\x00-\x7F]*$/.test(name) && name.trim()
    ? name.replace(/\s+/g, '').toLowerCase()
    : pick(NICKNAME_POOLS).replace(/\s+/g, '').toLowerCase()
  const safe = /^[a-z0-9._-]+$/.test(handle) ? handle : 'user'
  return `${safe}${randInt(10, 9999)}@${pick(DUMMY_EMAIL_DOMAINS)}`
}
/* 더미 휴대전화 번호.
   「전기통신번호관리세칙」(과기정통부 고시 제2021-93호) 제20조제4항제1호는
   이동전화번호를 `010-ABYY-YYYY(A=2～9)`로 정한다 — 즉 **가운데 블록 첫 자리가 0 또는 1인
...
export function uuid() {
  /* 브라우저에 표준 API가 있으면 그것을 쓴다 — RFC 9562는 CSPRNG 사용을 SHOULD로 권고한다.
     아래 Math.random 경로는 폴백이며 더미 표시용이다. */
...
/* ───────── UI 요소 생성 ───────── */
export const UI_ELEMENT_OPTIONS: { key: UIElement; icon: string; label: string }[] = [
...
/* ───────── JSON 더미 데이터 ───────── */
export const DATA_TYPE_OPTIONS: { key: DataType; icon: string; label: string }[] = [
...
export const FORMAT_OPTIONS: { key: FormatType; label: string }[] = [
  { key: 'json',     label: 'JSON' },
...
export type DataItem = Record<string, string | number | boolean | null>

export function generateOne(type: DataType): DataItem {
  switch (type) {
    case 'userProfile': {
    ...
    case 'event': {
      ...
      return {
        ...
        /* ⚠️ toISOString()은 UTC라 한국 시각보다 9시간 이르게 표시됐다 — 로컬 기준으로. */
        startsAt: localDateTimeStr(start),
        endsAt: localDateTimeStr(end),
...
export function generateMany(type: DataType, count: number): DataItem[] {
  return Array.from({ length: count }, () => generateOne(type))
}

/* ───────── 포맷터 ───────── */
export function toJSON(data: DataItem[]): string {
  return JSON.stringify(data, null, 2)
}
export function toJSONL(data: DataItem[]): string {
  return data.map(d => JSON.stringify(d)).join('\n')
}
export function toCSV(data: DataItem[]): string {
...
/** YAML 스칼라 인용 판정.
    ⚠️ 예전에는 /[:#-]/ 만 봐서 'yes'·'null'·'007'·'[대괄호]'·'* 별표'가 무인용으로 나갔다 —
       각각 불리언·널·숫자·배열·별칭으로 잘못 파싱된다. */
export function yamlScalar(v: unknown): string {
  ...
  const needsQuote =
    s === '' ||
    /^[\s]|[\s]$/.test(s) ||                                  // 앞뒤 공백
    /^[-?:,[\]{}#&*!|>'"%@`]/.test(s) ||                       // YAML 지시 문자로 시작
    /[:"'\\]/.test(s) ||                                        // 콜론·따옴표·백슬래시 — 파서에 따라 갈림
    /\s#/.test(s) ||                                            // 공백+# (주석으로 읽힘)
    /[\n\r\t]/.test(s) ||                                      // 제어문자
    /^(true|false|yes|no|on|off|null|~)$/i.test(s) ||           // 불리언·널로 읽힘
    /^[+-]?(\d[\d_]*(\.\d*)?([eE][+-]?\d+)?|\.inf|\.nan|0x[0-9a-fA-F]+|0o?[0-7]+)$/i.test(s)  // 숫자로 읽힘
  if (!needsQuote) return s
  return `"${s.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n').replace(/\t/g, '\\t')}"`
}
export function toYAML(data: DataItem[]): string {
  return data.map(d => {
    const lines = Object.entries(d).map(([k, v]) => `  ${k}: ${yamlScalar(v)}`).join('\n')
    return `-\n${lines}`
  }).join('\n')
}
export function toMarkdown(data: DataItem[]): string {
  if (data.length === 0) return ''
  const keys = Object.keys(data[0])
...
/** HTML 이스케이프. ⚠️ 예전에는 값을 <td>에 그대로 넣어 'AT&T <b>굵게</b>' 같은 값이
    표를 깨뜨렸다 — 생성된 코드를 그대로 붙여 쓰는 도구라 반드시 이스케이프해야 한다. */
...
export function toHTML(data: DataItem[]): string {
  if (data.length === 0) return ''
  const keys = Object.keys(data[0])
...
export function formatData(data: DataItem[], format: FormatType, type: DataType): string {
  switch (format) {
...
/* ───────── UX 시나리오 ───────── */
export const SCENARIOS: { key: Scenario; icon: string; label: string }[] = [
...
/* ───────── 길이 테스트 ───────── */
export const OVERFLOW_TESTS: { key: string; label: string; text: string }[] = [
