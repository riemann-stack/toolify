...
const FAQ_LD = [
  { "q":"마들렌 황금비율이 정확히 뭔가요?","a":"가장 클래식한 마들렌 비율: 계란 : 설탕 : 밀가루 : 버터 = 1 : 1 : 1 : 1 + 베이킹파우더 3%, 꿀 10%. 계란 100g 기준 → 설탕 100g · 밀가루 100g · 버터 95~100g · BP 3g · 꿀 10g. 변형: 브라운 버터(버터 110% + 꿀 12%), 레몬(레몬 제스트 5%), 초코(코코아 가루 10% + 우유 10%). 본 도구의 마들렌 모드 + 식감 슬라이더를 활용하세요." },
  ...
  { "q":"본 도구와 베이커 퍼센트 도구는 어떻게 다른가요?","a":"두 도구는 영역이 다릅니다. 🍞 베이커 퍼센트 계산기(/tools/cooking/baker-percent)는 빵 전용으로, 밀가루 100% 기준에 발효 시간·반죽 온도·이스트·발효종 중심입니다. 🧁 제과 레시피 계산기(본 도구)는 제과 전용으로, 품목별 자동 기준(계란·버터·흰자 등)에 비율 진단·식감 보정·굽기 중심입니다. 빵을 만들면 베이커 퍼센트, 디저트를 만들면 본 도구를 쓰세요." },
  { "q":"틀이 다른데 같은 레시피를 사용 가능한가요?","a":"본 도구의 &ldquo;분량 변환&rdquo; 탭을 활용하세요. 예: 표준 파운드(800g) 레시피 → 미니 파운드(200g) 3개로 만들면 ×0.75 (200×3=600g/800g). 18cm 원형 → 21cm 원형은 ×1.36. 다만 ① 굽는 시간은 별도 조정(작을수록 ↓·클수록 ↑) ② 마카롱·머랭처럼 민감한 품목은 신중히 환산해야 합니다." },
  { "q":"알레르기가 있어도 본 도구 결과를 사용해도 되나요?","a":"본 도구는 일반 가이드입니다. 알레르기 환자는 라벨 확인 필수이며, 의사·영양사 상담 후 진행하세요. 대체 가이드(참고용): 계란 → 아쿠아파바·플랙스에그 / 우유 → 두유·아몬드밀크 / 밀(글루텐) → 쌀가루·아몬드 가루 / 견과(아몬드) → 코코넛 가루 / 버터 → 코코넛 오일·식물성 유지. 단, 대체 시 식감이 크게 변할 수 있고, 본 도구의 결과로 자가 진단·자가 처방은 금물입니다. 영아·어린이 첫 시도는 의사와 상담 후 소량부터, 응급 약(에피펜 등)을 준비하세요." },
