...
const FAQ_LD = [
              ...
              },
              {
                ...
                a: '온도는 <strong>비선형 변환</strong>이기 때문입니다. 길이·무게는 단순히 곱셈/나눗셈으로 변환되지만 온도는 <strong>0점이 다릅니다</strong> — 섭씨 0℃는 화씨 32℉, 켈빈 273.15K. 따라서 ℉ = ℃ × 9/5 + <strong>32</strong> 같은 덧셈 항이 필요합니다.',
              },
              {
              ...
              },
              {
                ...
                a: '<strong>실온 보관용은 65°Bx 이상</strong>이 기준입니다. 당 농도가 이만큼 높아야 미생물 증식이 억제되기 때문입니다(시판 잼은 60~68°Bx). 설탕을 줄인 <strong>저당 수제 잼·청</strong>은 과일 맛은 살지만 발효·곰팡이 위험이 커서 냉장 보관하고 빨리 먹는 것이 기본입니다. 굴절계로 측정하면 정확하고, 굴절계가 없으면 과일 무게의 50~80%만큼 설탕을 넣고 졸이세요. 예: 딸기 1kg + 설탕 800g은 끓이기 전 약 48°Bx이므로, 실온 보관용은 65°Bx 이상이 될 때까지 더 졸입니다.',
              },
              {
              ...
              },
              {
                ...
                a: '「장애인·노인·임산부 등의 편의증진 보장에 관한 법률」 시행규칙 [별표 1]에 따라 <strong>경사로 기울기는 1/12 (≈ 8.3% 경사 ≈ 4.76°) 이하</strong>가 의무 기준입니다. 1/8(12.5%)까지 완화되는 것은 <strong>신축이 아닌 기존시설</strong>이면서 <strong>높이 1m 이하</strong>로 1/12 설치가 곤란하고 <strong>상시 보조서비스</strong>를 제공하는 경우뿐이며, 세 요건을 모두 갖춰야 합니다. 또 바닥면에서 높이 0.75m 이내마다 1.5m×1.5m 이상의 수평 휴식참을 두어야 합니다. 휠체어 자력 이동을 위한 최선은 1/16(6.25%) 이하이며, 가정용 이동식 경사로도 가능하면 1/12 이하가 되도록 길이를 확보하는 편이 안전합니다.',
              },
            ]
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
        ...
        <section>
          ...
          <p style={{ fontSize: '13px', color: 'var(--muted)', marginTop: '12px', lineHeight: 1.7 }}>
            ⓘ <strong style={{ color: 'var(--text)' }}>1근 주의</strong> — 정육점에서 쓰는 고기 1근은 600g이지만, 채소·과일 1근은 1관의 1/10인 375g이라 시장에서는 400g 정도로 어림합니다. 중국 1근은 500g이므로 중국 레시피를 볼 때도 구분해야 합니다.
          </p>
        </section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              <div key={i} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '11px 14px' }}>
                <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text)', marginBottom: '3px' }}>{c.icon} {c.name}</p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--muted)', fontWeight: 500 }}>활용</th>
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--muted)', fontWeight: 500 }}>표준 농도</th>
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--muted)', fontWeight: 500 }}>비고</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ['🍓 잼·청 (냉장 보관 수제)', '65 °Bx 미만', '당도가 낮을수록 발효·곰팡이 위험 ↑ · 냉장 보관'],
                  ['🍓 잼·청 (보존용)',   '65~68 °Bx',   '실온 보관 가능 (시판 잼은 60~68 °Bx)'],
        ...
        <section>
          ...
          <div style={{ overflowX: 'auto', marginBottom: 12 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--muted)', fontWeight: 500 }}>용도</th>
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--muted)', fontWeight: 500 }}>권장 농도</th>
                  <th scope="col" style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--muted)', fontWeight: 500 }}>희석 예시</th>
                </tr>
              </thead>
              <tbody>
                {[
