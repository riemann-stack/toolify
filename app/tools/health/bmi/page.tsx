          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {[
              ...
              <div key={i} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '11px 14px', display: 'grid', gridTemplateColumns: '32px 1fr auto', gap: 10, alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 12.5, color: 'var(--accent)', fontFamily: 'Syne, sans-serif', fontWeight: 800 }}>{g.range}</span>
              </div>
            ))}
          </div>
        ...
        <section>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  <th style={{ padding: '10px 12px', textAlign: 'left',   color: 'var(--muted)', fontWeight: 500 }}>측정법</th>
                  <th style={{ padding: '10px 12px', textAlign: 'center', color: 'var(--muted)', fontWeight: 500 }}>정확도</th>
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--bg2)' }}>
                    ...
                    <td style={{ padding: '10px 12px', textAlign: 'center', color: 'var(--accent)', fontFamily: 'Syne, sans-serif', fontWeight: 700 }}>{a}</td>
            ...
            {[
              {
                ...
                a: 'BMI는 체지방의 분포를 반영하지 않습니다. 근육량이 적고 복부에 지방이 집중된 경우 BMI가 정상이어도 복부비만일 수 있습니다. 허리둘레가 남성 90cm, 여성 85cm 이상이면 복부비만으로 판정합니다. 본 도구의 <strong>[허리둘레] 탭</strong>에서 BMI + 허리둘레를 종합해 "마른 비만"을 자동으로 감지합니다.',
              },
              {
                q: '운동선수나 근육량이 많은 사람의 BMI는 어떻게 해석하나요?',
                a: 'BMI는 근육과 지방을 구분하지 않아 근육량이 많은 운동선수는 BMI가 높게 나와도 실제로는 건강할 수 있습니다. 정확한 체성분 분석을 원한다면 인바디 검사나 체지방률 측정을 권장합니다. 본 도구의 <strong>[체지방률] 탭</strong>에서 Navy 공식 기반 추정값을 확인할 수 있습니다.',
              },
              {
              ...
              },
              {
                ...
                a: '본 도구는 입력한 키에 따라 자동으로 계산합니다 — 저체중(BMI 18.5 미만)·정상(18.5~22.9 한국 / 18.5~24.9 WHO)·과체중·비만 모든 구간을 본인 키 기준 kg 범위로 변환해 표시합니다. 예: 키 170cm → 정상 53.5~66.2kg (한국 기준). 결과 화면의 <strong>"키별 체중 구간 표"</strong>에서 모든 구간이 한눈에 보입니다.',
              },
              {
                q: '허리-신장비는 무엇이며 어떻게 활용하나요?',
                a: '허리-신장비(WHtR)는 허리둘레를 키로 나눈 비율입니다 (허리cm ÷ 키cm). 기준 — 0.43 미만(매우 마름)·0.43~0.49(이상적)·0.50~0.53(정상)·0.53~0.58(복부비만 주의)·0.58 이상(복부비만). <strong>"허리둘레가 키의 절반을 넘지 않도록"</strong>이라는 단순 원칙으로 기억하면 편리합니다. BMI보다 복부비만 위험을 직관적으로 파악할 수 있어 국제적으로 보조 지표로 자주 활용됩니다.',
              },
              {
