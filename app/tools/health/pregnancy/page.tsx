        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              {
                q: '임신 주수는 어떻게 계산하나요?',
                a: '임신 주수는 마지막 생리 시작일로부터 계산합니다. 실제 수정은 배란일(생리 시작 후 약 14일)에 일어나지만, 정확한 배란일을 알기 어렵기 때문에 의학적으로는 마지막 생리 시작일을 기준으로 삼습니다. 따라서 임신 1주차는 아직 수정 전인 시기입니다.',
              },
              {
                q: '출산 예정일은 어떻게 계산하나요?',
                a: '출산 예정일은 마지막 생리 시작일로부터 280일(40주) 후입니다. 네겔레 공식에 따라 마지막 생리 시작일에 7일을 더하고 3개월을 빼면 됩니다. 실제 출산은 예정일 ±2주 사이에 일어나는 경우가 많습니다.',
              },
              {
                q: '초음파 주수와 생리 기준 주수가 달라요!',
                a: '초기 초음파 검사에서 태아의 크기(CRL, 두부-둔부 길이)를 측정하여 주수를 보정할 수 있습니다. 이 경우 의사가 새로 지정해 준 주수가 더 정확합니다. 본 계산기는 생리 기준 주수를 사용하므로 초음파 결과와 1~2주 차이가 날 수 있으며, <strong>산부인과에서 보정받은 주수가 있다면 그 결과를 우선 적용</strong>하세요.',
              },
              {
                q: '임신 주수를 개월로 환산하면?',
                a: '의학적으로는 4주를 1개월로 봅니다. 예를 들어 28주는 7개월이 됩니다. 하지만 일반적인 달력 기준(30~31일/월)과는 차이가 있을 수 있으므로, 임신 경과는 주수(week)로 소통하는 것이 가장 정확합니다.',
              },
              {
                q: '임신 초기 증상에는 어떤 것이 있나요?',
                a: '착상혈(소량 출혈), 유방 팽창 및 압통, 입덧(메스꺼움·구토), 피로감, 빈뇨, 미각·후각 변화 등이 나타날 수 있습니다. 증상의 정도는 개인차가 매우 크며, 아무런 증상이 없는 경우도 있습니다. <strong>구체적 증상 해석은 산부인과 상담을 권장합니다.</strong>',
              },
              {
                q: '산전 검사는 꼭 다 받아야 하나요?',
                a: '본 도구가 안내하는 검사들은 표준 권장 사항이지만 모두 필수는 아닙니다.<br><br><strong>안전·태아 건강 직결 (필수에 가까움)</strong> — 첫 산전 검사 / 정밀 초음파 / 임신성 당뇨 검사 / GBS 검사.<br><br><strong>선택적 검사 (산모 상황·가족력에 따라)</strong> — 기형아 검사(NIPT·쿼드 등) / 양수 검사(고위험 시).<br><br>비용·시기·필요성은 담당 산부인과와 상담 후 결정하세요. 국민건강보험에서 일부 검사 비용을 지원합니다 (본인부담률 차이).',
              },
              {
                q: '태동을 언제부터 느끼나요?',
                a: '<strong>첫 임신(초산모)</strong>: 보통 18~22주차에 처음 느낌.<br><strong>경험 임신(경산모)</strong>: 보통 16~18주차에 더 빠름.<br><br>태동 종류:<br>· 초기: 가벼운 떨림·기포 터지는 느낌<br>· 중기: 분명한 발차기·움직임<br>· 후기: 강한 움직임·딸꾹질<br><br>28주차 이후는 매일 태동 횟수 기록을 권장하며, <strong>태동이 감소하면 즉시 산부인과 상담</strong>하세요.',
              },
              {
                q: '본 계산기 데이터는 어디에 저장되나요?',
                a: '입력한 정보(생리일·태명·체크리스트 진행)는 사용자 브라우저의 <strong>localStorage</strong>에 저장됩니다. 회원가입·로그인 불필요 / 사생활 보호 (서버 저장 X) / 같은 브라우저·기기에서만 접근 / 캐시 삭제 시 사라짐. 다른 기기 사용 시 출산 정보를 따로 백업하시기 바랍니다.',
              },
              {
                q: '쌍태아 임신은 어떻게 적용하나요?',
                a: '출산 예정일 계산은 동일하게 가능하지만 다음이 다릅니다.<br><br><strong>검진 빈도</strong> — 단태아: 4주 간격 → 후기 1~2주 / 쌍태아: 더 자주 (2~3주 간격).<br><strong>추가 검사</strong> — 추가 정밀 초음파 / 임신성 당뇨 위험↑ / 자궁경부 길이 측정(조산 위험↑).<br><strong>분만 시기</strong> — 단태아 평균 40주 / 쌍태아 평균 36~37주 (조산 빈도↑).<br><br>본 도구의 일반 일정은 단태아 기준이므로, 쌍태아 임신은 담당 산부인과의 일정 안내를 우선하세요.',
              },
              {
                q: '임신 중 카페인·약은 어떻게 해야 하나요?',
                a: '본 도구는 일반 가이드라인만 제공하며, 구체적 결정은 의료진 상담이 필수입니다.<br><br><strong>카페인</strong> — WHO 권장: 하루 200mg 이하 (커피 1~2잔). 너무 많은 카페인: 유산·저체중 위험↑.<br><br><strong>약물</strong> — 임신 전 복용 약: 즉시 산부인과 상담 / 처방약: 의사·약사에게 임신 알림 / 일반의약품(감기약·진통제 등): 1삼분기 피해야 할 약 多 / 한약·영양제: 안전성 확인 후.<br><br>&quot;임신 중 안전&quot; 표시도 본인 상황별로 다를 수 있으니 <strong>반드시 산부인과 또는 약사 상담 후 복용</strong>하세요.',
              },
            ].map((faq, i) => (
