    ...
    sections: [
      {
        h: '어떤 상황에 무엇을 쓰나요',
        body:
          '기념일·시험까지 남은 날은 D-day 계산기로, 정확한 나이는 만 나이 계산기로 확인합니다. 계약 기한처럼 공휴일을 빼고 세야 할 때는 D-day 계산기의 영업일 계산이, 유럽 여행을 준비할 때는 쉥겐 90/180일 체류 한도 계산기가 특히 유용합니다.',
      },
      {
        h: '법적 기간은 기관 기준과 함께',
        body:
...
export const CATEGORY_HUBS: Record<string, CategoryHub> = {
  finance: {
    ...
    groups: [
      { name: '급여·4대보험', blurb: '받는 돈을 확인할 때 — 월급, 4대보험, 알바 주휴수당, 연차, 퇴직금, 실업급여, 연금', hrefs: t('finance', 'salary', '4-insurance', 'hourly-pay', 'annual-leave', 'severance', 'unemployment-benefit', 'national-pension') },
      ...
      { name: '대출·부동산', blurb: '빌리고 사는 결정 — 대출 한도와 상환액, 취득세·중개보수, 전월세 비교, 청약, 경매', hrefs: t('finance', 'loan', 'dsr', 'acquisition-tax', 'brokerage-fee', 'rent-jeonse', 'housing-score', 'real-estate', 'auction') },
      { name: '투자·저축', blurb: '돈을 불리는 계산 — 예·적금 이자, 복리, 배당, 주식, 공모주, 금', hrefs: t('finance', 'deposit-interest', 'compound', 'savings', 'dividend', 'stock', 'ipo-deposit', 'installment', 'gold-converter', 'wealth-rank', 'historical-money') },
    ...
    situations: [
      ...
      { when: '퇴사', first: t('finance', 'severance'), next: t('finance', 'unemployment-benefit', 'annual-leave') },
      { when: '내 집 마련', first: t('finance', 'dsr'), next: t('finance', 'acquisition-tax', 'brokerage-fee', 'loan') },
      { when: '연초 세금 정리', first: t('finance', 'year-end-tax'), next: t('finance', 'freelance-tax') },
      { when: '목돈 모으기', first: t('finance', 'deposit-interest'), next: t('finance', 'compound', 'dividend') },
    ],
  },
  health: {
    h1: '건강·웰빙 계산기',
