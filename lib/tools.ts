...
export interface Category {
  ...
  name:  string
  color: string
  /** 소형 텍스트용 잉크 — 흰 배경 4.5:1 이상 보장 (globals.css --cat-*-ink와 단일 소스) */
  ink: string
  /** 홈 벤토 타일 등에 노출되는 대표 도구 요약 한 줄 (실존 도구만 인용) */
  tagline?: string
  tools: Tool[]
}
...
export const categories: Category[] = [
  {
    id: 'finance', icon: '💰', name: '금융·재테크', color: 'var(--cat-finance)', ink: 'var(--cat-finance-ink)', tagline: '연봉 · 대출 · 연말정산 · 배당',
    tools: [
  ...
  {
    id: 'health', icon: '🏃', name: '건강·웰빙', color: 'var(--cat-health)', ink: 'var(--cat-health-ink)', tagline: 'BMI · 칼로리 · 임신 주수',
    tools: [
      ...
      { href: '/tools/health/child-height',    icon: '📏', name: '자녀 예상 키 계산기', desc: '부모 키로 아들·딸의 예상 성인 키를 중간부모키(MPH) 공식으로 계산 + ±8.5cm 예측 범위. 통계 추정이며 의학 진단 아님.', badge: 'new' },
      { href: '/tools/health/heat-hydration',  icon: '💧', name: '폭염 수분·전해질 계산기', desc: '체중·활동·폭염 특보 단계로 오늘 마셔야 할 물의 양과 음용 패턴 + 전해질 보충 시점·저나트륨혈증 주의. 2026 폭염 3단계 반영.', badge: 'new' },
      { href: '/tools/health/hba1c',           icon: '🩸', name: '당화혈색소 변환기', desc: '건강검진 당화혈색소(HbA1c %)를 추정 평균혈당(eAG mg/dL·mmol/L)으로 양방향 변환 + 정상·당뇨 전단계·당뇨 진단 구간과 환산표.', badge: 'new' },
      { href: '/tools/health/glycemic-load',   icon: '🍚', name: '당부하지수(GL) 계산기', desc: '음식·섭취량으로 당부하지수(GL) 계산 + 한 끼 총 GL 합산 + 밥·과일·간식 GI 조회표. 저·중·고 판정으로 혈당 관리 참고.', badge: 'new' },
    ],
  },
  {
    id: 'cooking', icon: '🍳', name: '요리·식품', color: 'var(--cat-cooking)', ink: 'var(--cat-cooking-ink)', tagline: '레시피 · 베이킹 · 커피',
    tools: [
  ...
  {
    id: 'life', icon: '🎲', name: '생활·재미', color: 'var(--cat-life)', ink: 'var(--cat-life-ink)', tagline: '로또 · 더치페이 · 사다리',
    tools: [
      ...
      { href: '/tools/life/height-rank',   icon: '📏', name: '키 백분위 계산기', desc: '내 키 상위 몇 %? 사이즈코리아 8차 실측 통계(성별·연령대별) + 100명 중 몇 번째 + 분포 곡선 위 내 위치.', badge: 'new' },
      { href: '/tools/life/residual-value', icon: '⚖️', name: '잔존가치·보상액 계산기', desc: '수리불가·분실·세탁사고 배상액을 소비자분쟁해결기준(공정위 고시) 원문대로 — 가전 정액감가+10% 가산, 세탁물 배상비율표.', badge: 'new' },
    ],
  },
  {
    id: 'sports', icon: '⛳', name: '스포츠', color: 'var(--cat-sports)', ink: 'var(--cat-sports-ink)', tagline: '러닝 페이스 · 골프 · 등산',
    tools: [
      ...
      { href: '/tools/sports/park-golf', icon: '⛳', name: '파크골프 스코어카드', desc: '9홀 파33·18홀 최대 4인 실시간 합계·순위 + OB 2벌타 규칙·더블파 컷 로컬룰 + 라운드 기록 저장.', badge: 'new' },
      { href: '/tools/sports/shoe-mileage',    icon: '👟', name: '러닝화 수명 계산기', desc: '주간 거리·체중·미드솔 소재(EVA·TPU·PEBA)로 러닝화 예상 수명(km)과 교체 예상일 + 2족 로테이션 연장 효과.', badge: 'new' },
      { href: '/tools/sports/swim-pace',       icon: '🏊', name: '수영 페이스·SWOLF 계산기', desc: '100m 페이스 ↔ 총기록 양방향 + 25m·50m 풀 왕복수 환산 + SWOLF·인터벌 send-off·영법별 예상기록.', badge: 'new' },
  ...
  {
    id: 'interior', icon: '🏠', name: '주거·인테리어', color: 'var(--cat-interior)', ink: 'var(--cat-interior-ink)', tagline: '평수 · 도배 · 에어컨 평형',
    tools: [
      ...
      { href: '/tools/interior/dehumidifier',  icon: '💧', name: '제습기 용량 계산기',         desc: '평수·주거유형으로 적정 제습량(L/일)과 권장 제품 용량 + 월 전기요금. 지하·장마철 보정.', badge: 'new' },
  ...
  {
    id: 'unit', icon: '📐', name: '단위·변환', color: 'var(--cat-unit)', ink: 'var(--cat-unit-ink)', tagline: '평↔㎡ · 사이즈 · 연비',
    tools: [
  ...
  {
    id: 'art', icon: '🎨', name: '예술·창작', color: 'var(--cat-art)', ink: 'var(--cat-art-ink)', tagline: '음악 · 색상 · 글자수',
    tools: [
      /* 🎵 음악 */
  ...
  {
    id: 'edu', icon: '🔬', name: '교육·학습', color: 'var(--cat-edu)', ink: 'var(--cat-edu-ink)', tagline: '학점 환산 · 복습 간격',
    tools: [
  ...
  },
  {
    ...
    tools: [
      ...
      { href: '/tools/dev/llm-vram', icon: '🧠', name: '로컬 LLM VRAM 계산기', desc: 'GGUF 양자화별 가중치 + KV 캐시 + 오버헤드로 필요 VRAM 계산 — Llama·Qwen·Gemma·EXAONE 프리셋 + RTX·Mac 적합 판정.', badge: 'new' },
